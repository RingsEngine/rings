...
export class BitmapTextureCube extends TextureCube {
  ...
  protected generateImages(
    images: HTMLCanvasElement[] | ImageBitmap[] | OffscreenCanvas[] | Texture[]
  ) {
    ...
    for (let i = 1; i < this.mipmapCount; i++) {
      ...
      for (let faceId = 0; faceId < 6; faceId++) {
        ...
        faceTextures[faceId] = BlurTexture2DBufferCreator.blurImageFromTexture(
          dstBuffer,
          mipWidth,
          mipHeight,
          false
  ...
  private uploadBaseImages(
    size: number,
    textures: HTMLCanvasElement[] | ImageBitmap[] | OffscreenCanvas[]
  ) {
    let device = webGPUContext.device;
  ...
  public set images(
    ...
    if (this._images[0] instanceof HTMLImageElement) {
      ...
      if (
        this._images instanceof HTMLCanvasElement ||
        this._images instanceof ImageBitmap
      ) {
