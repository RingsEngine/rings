...
export class QuadTree {
  ...
  public buildQuadTree(maxNodesPerCell: number, minCellSize: number): void {
    ...
    while (cellsToProcess.length != 0) {
      ...
      if (
        this._cells[cellIndex].nodeIndices.length <= maxNodesPerCell ||
      ...
      for (i = 0; i < QuadTreeCell.NUM_CHILDREN; i++) {
        ...
        this._cells.push(
          new QuadTreeCell(this.createAABox(this._cells[cellIndex].aabb, i))
        );
  ...
  private doesNodeIntersectCell(node: IQuadNode, cell: QuadTreeCell): Boolean {
    ...
    if (!node.isTriangle) return true;
  ...
  private logTree(cellIndex: number): void {
    ...
    var spaces: String = "";
    for (var si: number = 0; si < this.logDeep - 1; si++) spaces += "-|";
    ...
    console.log(
      spaces +
        ...
        cellIndex +
        " " +
        cell.aabb.minPosX.toFixed(2) +
        " " +
        cell.aabb.maxPosX.toFixed(2) +
        " " +
        cell.aabb.minPosY.toFixed(2) +
        " " +
        cell.aabb.maxPosY.toFixed(2)
    );
      ...
      if (cell.nodeIndices[i] >= 0) {
        ...
        console.log(
          spaces +
            ...
            cell.nodeIndices[i] +
            " " +
            tri.aabb.minPosX.toFixed(2) +
            " " +
            tri.aabb.maxPosX.toFixed(2) +
            " " +
            tri.aabb.minPosY.toFixed(2) +
            " " +
            tri.aabb.maxPosY.toFixed(2)
        );
      }
    }
