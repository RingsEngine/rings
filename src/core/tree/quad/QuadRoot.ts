...
export class QuadRoot {
  ...
  public getNodesByAABB(
    ...
    var numNodes: number = this._quadTree.getNodesIntersectingtAABox(
      this._collisionNodesIdx,
      this._segBox
    );
  ...
  public getTriangleAtPoint(point: Vector3, threshold: number = 5): IQuadNode {
    ...
    var numTriangles: number = this._quadTree.getNodesIntersectingtAABox(
      this._collisionNodesIdx,
      this._segBox
    );
    ...
    for (var i: number = 0; i < this._collisionNodesIdx.length; i++) {
      ...
      if (curDistance > threshold) continue;
