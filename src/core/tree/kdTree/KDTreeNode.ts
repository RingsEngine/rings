...
export class KDTreeNode extends KDTreeUUID {
  ...
  protected autoSplit(): void {
    if (
      this._entities.count > KDTreeConfig.MaxEntityCountInLeaf &&
      !this._right &&
      !this._left &&
      this.layer < KDTreeConfig.MaxLayer
    ) {
  ...
  protected isEmpty(): boolean {
    return (
      this._left == null && this._right == null && this._entities.count == 0
    );
  }
  ...
  public autoClear(): void {
    ...
    while (
      that &&
      that.layer > KDTreeConfig.ClearLeafLayer &&
      that.clearLeaf()
    ) {
      that = that._parent;
    }
  ...
  private static rangeBox: BoundingBox = new BoundingBox(
    ...
    if (this._entities.count > 0) {
      ...
      for (let key in map) {
        ...
        if (!success && squareDistance > 0) {
          success =
            entity.squareDistanceTo(point, this._dimensions) <= squareDistance;
        }
        if (success) {
          ret.push(entity);
        }
