...
export class GeometryVertexBuffer {
  ...
  private createComposeVertexBuffer(
    ...
    this.vertexGPUBuffer = new VertexGPUBuffer(
      this.vertexCount * attributeOffset
    );
  }
      ...
      case GeometryVertexType.split:
        {
          ...
          this.vertexGPUBuffer.node.setFloat32Array(
            vertexBufferLayout.offset / 4,
            vertexDataInfo.data as Float32Array
          );
        }
        break;
      case GeometryVertexType.compose:
        {
          for (let i = 0; i < this.vertexCount; i++) {
            const attributeLayout =
              this._attributeSlotLayouts[0][this._attributeLocation[attribute]];
            for (let k = 0; k < attributeLayout.stride; k++) {
              let atData = vertexDataInfo.data[i * attributeLayout.stride + k];
              let index =
                i * (this._vertexBufferLayouts[0].arrayStride / 4) +
                attributeLayout.offset / 4 +
                k;
      ...
      case GeometryVertexType.split:
        {
          for (let i = 0; i < this._vertexBufferLayouts.length; i++) {
            ...
            this.vertexGPUBuffer.node.setFloat32Array(
              vertexBufferLayout.offset / 4,
              attributeData.data as Float32Array
            );
          }
        }
        break;
          ...
          for (let i = 0; i < this.vertexCount; i++) {
            this._attributeSlotLayouts.forEach((v) => {
              for (let j = 0; j < v.length; j++) {
                ...
                for (let k = 0; k < attributeLayout.stride; k++) {
                  let atData =
                    attributeData.data[i * attributeLayout.stride + k];
                  let index =
                    i * (this._vertexBufferLayouts[0].arrayStride / 4) +
                    attributeLayout.offset / 4 +
                    k;
  ...
  public destroy(force?: boolean) {
    ...
    if (this.vertexGPUBuffer) this.vertexGPUBuffer.destroy(force);
