...
export class GeometryBase {
  ...
  generate(shaderReflection: ShaderReflection) {
    if (this._onChange) {
      ...
      this._indicesBuffer.upload(
        this.getAttribute(VertexAttributeName.indices).data
      );
  ...
  public genWireframe(): Vector3[] {
    ...
    if (
      this.geometryType == GeometryVertexType.split ||
      ...
      if (indexAttribute && vertexAttribute && indexAttribute.data.length > 0) {
        ...
        for (let i = 0; i < indexAttribute.data.length / 3; i++) {
          ...
          let p1 = new Vector3(
            vertexData[i1 * strip + 0],
            vertexData[i1 * strip + 1],
            vertexData[i1 * strip + 2]
          );
          let p2 = new Vector3(
            vertexData[i2 * strip + 0],
            vertexData[i2 * strip + 1],
            vertexData[i2 * strip + 2]
          );
          let p3 = new Vector3(
            vertexData[i3 * strip + 0],
            vertexData[i3 * strip + 1],
            vertexData[i3 * strip + 2]
          );
  ...
  public computeNormals(): this {
    ...
    for (let i = 0; i < trianglesCount; i++) {
      ...
      point1.set(
        posAttrData.data[index1 * 3],
        posAttrData.data[index1 * 3 + 1],
        posAttrData.data[index1 * 3 + 2]
      );
      point2.set(
        posAttrData.data[index2 * 3],
        posAttrData.data[index2 * 3 + 1],
        posAttrData.data[index2 * 3 + 2]
      );
      point3.set(
        posAttrData.data[index3 * 3],
        posAttrData.data[index3 * 3 + 1],
        posAttrData.data[index3 * 3 + 2]
      );
      ...
      normalAttrData.data[index1 * 3] =
        normalAttrData.data[index2 * 3] =
        normalAttrData.data[index3 * 3] =
          normal.x;
      normalAttrData.data[index1 * 3 + 1] =
        normalAttrData.data[index2 * 3 + 1] =
        normalAttrData.data[index3 * 3 + 1] =
          normal.y;
      normalAttrData.data[index1 * 3 + 2] =
        normalAttrData.data[index2 * 3 + 2] =
        normalAttrData.data[index3 * 3 + 2] =
          normal.z;
    }
