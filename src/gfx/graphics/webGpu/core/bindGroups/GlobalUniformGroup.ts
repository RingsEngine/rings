...
export class GlobalUniformGroup {
  ...
  constructor(matrixBindGroup: MatrixBindGroup) {
    ...
    this.uniformGPUBuffer.visibility =
  ...
  createBindGroup() {
    this.uniformByteLength =
      this.uniformGPUBuffer.memory.shareDataBuffer.byteLength;
  ...
  public setCamera(camera: Camera3D) {
    this.uniformGPUBuffer.setMatrix(
      `_projectionMatrix`,
    ...
    let shadowLightList = ShadowLightsCollect.getDirectShadowLightWhichScene(
      camera.transform.scene3D
    );
    ...
    let reflectionCount = EntityCollect.instance.getReflections(
      camera.transform.scene3D
    ).length;
    this.uniformGPUBuffer.setFloat(
      `reflectionProbeSize`,
      reflectionSetting.reflectionProbeSize
    );
    this.uniformGPUBuffer.setFloat(
    ...
    this.uniformGPUBuffer.setVector3(
      `CameraPos`,
    ...
    this.uniformGPUBuffer.setFloat(
      `shadowBias`,
      camera.getShadowBias(shadowMapSize)
    );
    this.uniformGPUBuffer.setFloat(
      `skyExposure`,
    ...
    this.uniformGPUBuffer.setFloat(
      `renderPassState`,
      Engine3D.setting.render.renderPassState
    );
    this.uniformGPUBuffer.setFloat(
      `quadScale`,
      Engine3D.setting.render.quadScale
    );
    this.uniformGPUBuffer.setFloat(
      `hdrExposure`,
      Engine3D.setting.render.hdrExposure
    );
    this.uniformGPUBuffer.setInt32(
      `renderState_left`,
      Engine3D.setting.render.renderState_left
    );
    this.uniformGPUBuffer.setInt32(
      `renderState_right`,
      Engine3D.setting.render.renderState_right
    );
    this.uniformGPUBuffer.setFloat(
      `renderState_split`,
      Engine3D.setting.render.renderState_split
    );
    ...
    this.uniformGPUBuffer.setVector4Array(
      `frustumPlanes`,
  ...
  setShadowCamera(camera: Camera3D) {
    ...
    this.uniformGPUBuffer.setFloat(
      `shadowBias`,
      Engine3D.setting.shadow.shadowBias
    );
    this.uniformGPUBuffer.setFloat(
      `skyExposure`,
      Engine3D.setting.sky.skyExposure
    ...
    this.uniformGPUBuffer.setFloat(
      `shadowMapSize`,
      Engine3D.setting.shadow.shadowSize
    );
    this.uniformGPUBuffer.setFloat(
      `shadowSoft`,
      Engine3D.setting.shadow.shadowSoft
