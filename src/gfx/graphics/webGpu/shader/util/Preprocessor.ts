...
export class Preprocessor {
  ...
  public static parseComputeShader(
    code: string,
    defineValue: { [name: string]: any }
  ): string {
    code = this.filterComment(code);
  ...
  protected static parsePreprocess(
  ...
  protected static parseAutoBindingForAllGroup(code: string): string {
    ...
    while (offset < code.length) {
      ...
      let groupID = Number.parseInt(
        code.substring(nLeftIndex + 7, nRightIndex)
      );
  ...
  protected static parseAutoBindingForGroupX(
    code: string,
    nGroup: number
  ): string {
    let offset = 0;
  ...
  protected static parsePreprocessCommand(
    context: PreprocessorContext,
    code: string,
    defineValue: { [name: string]: any }
  ): string {
    ...
    for (let i: number = 0; i < lines.length; i++) {
      ...
      if (command.indexOf("#if") != -1) {
        ...
        command.indexOf("#elseif") != -1 ||
        (command.indexOf("#else") != -1 && command.indexOf(" if") != -1)
      ) {
        ...
        if (condition == "") {
          console.error(
            `preprocess command error, conditions missing: ${command}`
          );
        }
        skip = !this.parseCondition(condition, defineValue);
        stack.push(skip);
        ...
        let expression = command
          .substring(command.indexOf("#define ") + 8)
          .trim();
  ...
  protected static parseCondition(
    condition: string,
    defineValue: { [name: string]: any }
  ): boolean {
