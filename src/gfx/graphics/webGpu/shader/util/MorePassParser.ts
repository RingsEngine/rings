...
export class MorePassParser {
  ...
  public static parser(
    ...
    let block = code
      .substring(index1 + this.shaderKeyword.length, index2)
      .trim();
    ...
    for (let passCode of passBlocks) {
      ...
      if (passShader.vertexShader.length > 0) {
        passShader.vertexShader = Preprocessor.parse(
          passShader.vertexShader,
          defineValue
        );
      }
      if (passShader.fragmentShader.length > 0) {
        passShader.fragmentShader = Preprocessor.parse(
          passShader.fragmentShader,
          defineValue
        );
      }
    }
  ...
  protected static parserPassBlock(code: string): PassShader {
    ...
    if (index1 != -1) {
      passShader.vertexShader = this.extractBlock(
        code.substring(index1 + this.vertexKeyword.length),
        "{",
        "}"
      );
    }
    ...
    if (index1 != -1) {
      passShader.fragmentShader = this.extractBlock(
        code.substring(index1 + this.fragmentKeyword.length),
        "{",
        "}"
      );
    }
  ...
  protected static parserShaderState(
  ...
  protected static convertValue(str: string): number | boolean | string {
    if (str.length == 4 && str.toLowerCase() == "true") return true;
    else if (str.length == 5 && str.toLowerCase() == "false") return false;
    else if (str[0] == '"') return str.substring(1, str.length - 1);
