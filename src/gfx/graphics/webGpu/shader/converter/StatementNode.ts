...
export class SN_Struct extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Struct {
    if (
      r.peekToken(0).Type == TokenType.IDENT &&
...
export class SN_Function extends StatementNode {
  ...
  public formatToWGSL(context: TranslatorContext, depth: number): string {
    ...
    let hasOutStruct: boolean =
      context.layoutsOut.length > 0 || context.builtinOut.length > 0;
    ...
    for (let statement of this.body.nodes) {
      ...
      if (
        !(statement instanceof SN_IFBranch) &&
...
export class SN_Declaration extends StatementNode {
  ...
  constructor(
    type: string,
    name: string,
    arraySize: SN_Expression = new SN_Expression()
  ) {
    super();
    this.type = type;
    this.name = name;
  ...
  public static parse(r: GLSLLexer): SN_Declaration {
    ...
    // inout
      ...
      throw (
        "Error parsing declaration expression: Unexpected character(" +
        first.Literal +
        ")"
      );
    }
    ...
    if (first.Type == TokenType.SEMICOLON) {
      ...
      // float a, b = 0, c = 1 + 2
      while (
        r.peekToken(0).Type == TokenType.COMMA &&
        r.peekToken(1).Type == TokenType.IDENT
      ) {
        let declaration = new SN_Declaration(
          result.type,
          r.peekToken(1).Literal
        );
...
export class SN_ForLoop extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_ForLoop {
    if (
      r.peekToken(0).Type == TokenType.FOR &&
      r.peekToken(1).Type == TokenType.LEFTSAMLL
    ) {
      r.skipToken(2);
      ...
      if (r.peekToken(0).Type == TokenType.SEMICOLON) {
        ...
        expression1 = new SN_Declaration(
          r.peekToken(0).Literal,
          r.peekToken(1).Literal
        );
        r.skipToken(1);
...
export class SN_Expression extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Expression {
    ...
    while (r.peekToken(0).Type != TokenType.EOF) {
      ...
      if (!currToken.isOperation()) {
        if (currToken.Type == TokenType.LITERAL) {
          ...
          // { 1.0, 2.0, 3.0 }
          if (
            currToken.Type == TokenType.LEFTBIG &&
          ...
          if (currToken.Type == TokenType.QUEMARK) {
            if (opStack.length > 0) {
              ...
              if (
                opToken.nOperationPriorityLevel <=
        ...
        if (
          ...
          let opToken = opStack[opStack.length - 1];
          if (
            opToken.nOperationPriorityLevel <= currToken.nOperationPriorityLevel
          ) {
            SN_Expression.unionOperation(opStack, valueStack);
          } else if (
            opStack.length > 0 &&
    ...
    throw (
      "Error parsing expression: Unexpected character(" +
      r.peekToken(0).Literal +
      ")"
    );
  }
  ...
  protected static unionOperation(
    opStack: Array<GLSLLexerToken>,
...
export class SN_ParenExpression extends StatementNode {
  ...
  public formatToWGSL(context: TranslatorContext, depth: number): string {
    let prefix: string = depth <= 0 ? "" : "    ".repeat(depth);
    let result: string =
      prefix + "(" + this.nodes[0].formatToWGSL(context, 0) + ")";
    return result;
  }
}
...
export class SN_Constant extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Constant {
    ...
    throw (
      "Error parsing literal constants: Unexpected characters(" +
      firstToken.Literal +
      ")"
    );
  }
...
export class SN_Continue extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Continue {
    if (
      r.peekToken(0).Type == TokenType.CONTINUE &&
...
export class SN_UnaryOperation extends StatementNode {
  ...
  constructor(
    op: GLSLLexerToken,
    lValue: StatementNode | undefined,
    rValue: StatementNode | undefined
  ) {
    super();
    this.op = op;
...
export class SN_BinaryOperation extends StatementNode {
  ...
  constructor(
    op: GLSLLexerToken,
    lValue: StatementNode,
    rValue: StatementNode
  ) {
    super();
    this.op = op;
    this.leftValue = lValue;
  ...
  public static parse(r: GLSLLexer): SN_CodeBlock {
    if (r.peekToken(0).Type == TokenType.LEFTBIG) {
      ...
      for (let count = 1; count > 0 && r.peekToken(0).Type != TokenType.EOF; ) {
         ...
         */
        if (
          currToken.Type == TokenType.CONST &&
          ...
          let op = r.peekToken(1);
          result.addNode(
            new SN_UnaryOperation(
              op,
              new SN_Identifier(currToken.Literal),
              undefined
            )
          );
          r.skipToken(3);
        ...
        throw (
          "Error parsing block: Unexpected symbol(" + currToken.Literal + ")"
        );
      }
...
export class SN_Precision extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Precision {
    if (
      r.peekToken(0).Type == TokenType.PRECISION &&
      r.peekToken(1).Type == TokenType.IDENT &&
      r.peekToken(2).isBuiltinType()
    ) {
      let result = new SN_Precision(
        r.peekToken(1).Literal,
        r.peekToken(2).Literal
      );
      r.skipToken(3);
      if (r.peekToken(0).Type == TokenType.SEMICOLON) {
        r.skipToken(1);
...
export class SN_Layout extends StatementNode {
  ...
  public static parse(r: GLSLLexer): SN_Layout {
    if (
      r.peekToken(0).Type == TokenType.LAYOUT &&
      r.peekToken(1).Type == TokenType.LEFTSAMLL
    ) {
      ...
      // structure type
        ...
        if (
          ...
          structNode.addNode(
            new SN_Declaration(structNode.name, r.peekToken(0).Literal)
          );
          r.skipToken(2);
        }
        ...
        throw (
          "Error parsing layout qualifier type: Unexpected symbol(" +
          r.peekToken(0).Literal +
          ")"
        );
    }
