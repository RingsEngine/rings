...
export class ShaderReflection {
  private static _shaderReflectionMap: Map<string, ShaderReflection> = new Map<
    string,
    ShaderReflection
  >();
  ...
  public static parser2(wgsl: string, shaderBase: ShaderPassBase) {
    ...
    if (wgsl.indexOf(`@vertex`) != -1) {
      shaderReflection.attributes = this.parserVertex(
  ...
  public static combineShaderReflectionVarInfo(
      ...
      if (shaderReflection.groups[iterator.group][iterator.binding]) {
        ...
        if (import.meta.env.DEV) {
          if (aInfo.varName != bInfo.varName)
            console.warn(
              `shader reflection var not match! var name vs : ${aInfo.varName} , fs : ${bInfo.varName}`
            );
          if (aInfo.varType != bInfo.varType)
            console.error(
              `shader reflection varType not match! var varType vs : ${aInfo.varType} , fs : ${bInfo.varType}`
            );
          if (aInfo.dataType != bInfo.dataType)
            console.warn(
              `shader reflection dataType not match! var dataType vs : ${aInfo.dataType} , fs : ${bInfo.dataType}`
            );
          if (aInfo.dataIsBuiltinType != bInfo.dataIsBuiltinType)
            console.error(
              `shader reflection dataIsBuiltinType not match! var dataIsBuiltinType vs : ${aInfo.dataIsBuiltinType} , fs : ${bInfo.dataType}`
            );
          if (!aInfo.dataFields || !bInfo.dataFields) {
            console.warn(
              `shader reflection dataFields is empty! var dataFields vs : ${aInfo.dataFields} , fs : ${bInfo.dataFields}`
            );
          }
        }
  ...
  public static final(shaderBase: ShaderPassBase) {
    ...
    this.combineShaderReflectionVarInfo(
      shaderReflection,
      shaderReflection.vs_variables
    );
    this.combineShaderReflectionVarInfo(
      shaderReflection,
      shaderReflection.fs_variables
    );
  }
        ...
        if (index != -1) {
          ...
          switch (type) {
            ...
            case "mat4":
              return true;
            case "array":
              return this.isBuiltinTypes(
                dataType.substring(index + 1, dataType.lastIndexOf(">"))
              );
          }
        }
        break;
