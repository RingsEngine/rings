...
export class RenderShaderPass extends ShaderPassBase {
  ...
  constructor(vs: string, fs: string) {
    ...
    if (!(this.vsName in ShaderLib)) {
      console.error(
        `Shader Not Register, Please Register Shader!`,
        this.vsName
      );
    }
    ...
    this.materialDataUniformBuffer.visibility =
      ...
      if (info.varType == `uniform`) {
        ...
        switch (info.dataType) {
          ...
          case `texture_depth_cube_array`:
            {
              let texture = this.textures[info.varName]
                ? this.textures[info.varName]
                : Engine3D.res.redTexture;
              let entry: GPUBindGroupLayoutEntry = {
                binding: info.binding,
                visibility: texture.visibility,
      ...
      for (let j = 0; j < shaderRefs.length; j++) {
        ...
        if (refs.varType == `uniform`) {
          ...
          if (buffer) {
            ...
            console.error(
              `shader${this.vsName}-${this.fsName}`,
              `buffer ${refs.varName} is missing!`
            );
          }
  ...
  private createPipeline(
    ...
    if (renderPassState.zPreTexture || renderPassState.depthTexture) {
      ...
      if (
        Engine3D.setting.render.zPrePass &&
        renderPassState.zPreTexture &&
        shaderState.useZ
      ) {
    ...
    if (pipeline) {
      ...
      this.pipeline = GPUContext.createPipeline(
        renderPipelineDescriptor as GPURenderPipelineDescriptor
      );
  ...
  private createGroupLayouts() {
    ...
    this.bindGroupLayouts = [
      GlobalBindGroupLayout.getGlobalDataBindGroupLayout(),
    ];
  ...
  private preDefine(geometry: GeometryBase) {
    ...
    let hasMorphTarget = geometry.hasAttribute(
      VertexAttributeName.a_morphPositions_0
    );
    ...
    this.defineValue[`USE_HARD_SHADOW`] =
      Engine3D.setting.shadow.type == `HARD`;
    this.defineValue[`USE_SOFT_SHADOW`] =
