...
export class SSRPost extends PostBase {
  ...
  private createRayTraceShader() {
    ...
    this.SSR_RayTraceCompute.setUniformBuffer(
      "globalUniform",
    ...
    this.SSR_RayTraceCompute.setStorageBuffer(
      `rayTraceBuffer`,
      this.rayTraceData
    );
    this.SSR_RayTraceCompute.setStorageBuffer(
    ...
    this.SSR_RayTraceCompute.setSamplerTexture(
      "gBufferTexture",
      gBufferTexture
    );
    ...
    if (EntityCollect.instance.sky instanceof SkyRenderer)
      this.SSR_RayTraceCompute.setSamplerTexture(
        `prefilterMap`,
        EntityCollect.instance.sky.map
      );

    this.SSR_RayTraceCompute.workerSizeX = Math.ceil(
      this.isRetTexture.width / 8
    );
    this.SSR_RayTraceCompute.workerSizeY = Math.ceil(
      this.isRetTexture.height / 8
    );
  ...
  private createBlendShader(input: VirtualTexture): void {
    ...
    this.SSR_Blend_Compute.setStorageBuffer(
      `rayTraceBuffer`,
      this.rayTraceData
    );
    this.SSR_Blend_Compute.setUniformBuffer(
      "globalUniform",
      globalUniform.uniformGPUBuffer
    );
    ...
    this.SSR_Blend_Compute.workerSizeY = Math.ceil(
      this.finalTexture.height / 8
    );
  ...
  private createResource() {
    ...
    this.finalTexture = new VirtualTexture(
      w,
      h,
    ...
    let ssrWidth = Math.ceil(
      w * Engine3D.setting.render.postProcessing.ssr.pixelRatio
    );
    let ssrHeight = Math.ceil(
      h * Engine3D.setting.render.postProcessing.ssr.pixelRatio
    );

    this.isRetTexture = new VirtualTexture(
      ssrWidth,
      ssrHeight,
      GPUTextureFormat.rgba16float,
      false,
    ...
    this.rayTraceData = new StorageGPUBuffer(
      ssrWidth * ssrHeight * 8,
      GPUBufferUsage.COPY_SRC
    );
    this.ssrColorData = new StorageGPUBuffer(
      ssrWidth * ssrHeight * 4,
      GPUBufferUsage.COPY_SRC
    );
    this.historyPosition = new StorageGPUBuffer(
      ssrWidth * ssrHeight * 4,
      GPUBufferUsage.COPY_SRC
    );
  ...
  render(view: View3D, command: GPUCommandEncoder) {
    ...
    this.ssrUniformBuffer.setFloat(
      "roughnessThreshold",
      setting.roughnessThreshold
    );
