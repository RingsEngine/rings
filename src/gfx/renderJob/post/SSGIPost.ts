...
export class SSGIPost extends PostBase {
  ...
  onAttach(view: View3D) {
    this.view = view;
    view.camera.transform.onPositionChange = view.camera.transform
      .onPositionChange
      ? mergeFunctions(view.camera.transform.onPositionChange, (a, b) =>
          this.onCameraChange(a, b)
        )
      : (a, b) => this.onCameraChange(a, b);
  }
  ...
  private createResource() {
    ...
    this.lastPosTexture = new VirtualTexture(
      w,
      h,
    ...
    this.newTexture = new VirtualTexture(
      inW,
      inH,
      GPUTextureFormat.rgba16float,
      false,
    ...
    this.combineTexture = new VirtualTexture(
      inW,
      inH,
      GPUTextureFormat.rgba16float,
      false,
    ...
    ssgiDec.loadOp = `load`;
    this.rtFrame = new RTFrame([this.outTexture], [ssgiDec]);
  }
  ...
  private createCompute() {
    ...
    let globalUniform = GlobalBindGroup.getCameraGroup(this.view.camera);
    {
      ...
      this.ssgiCompute.setUniformBuffer(
        "globalUniform",
...

    {
      ...
      this.delayCompute.setStorageTexture(
        `combineTexture`,
        this.combineTexture
      );
    ...
    {
      this.combineCompute.setSamplerTexture(
        `inputBTexture`,
        this.combineTexture
      );
      this.combineCompute.setSamplerTexture(
      ...
      this.combineCompute.setUniformBuffer(
        "globalUniform",
        globalUniform.uniformGPUBuffer
      );
