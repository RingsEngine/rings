...
export class GodRayPost extends PostBase {
  ...
  private createCompute(view: View3D) {
    ...
    this.historyGodRayData = new StorageGPUBuffer(
      4 * this.godRayTexture.width * this.godRayTexture.height
    );
    this.godRayCompute.setStorageBuffer(
      "historyGodRayData",
      this.historyGodRayData
    );

    let rtFrame = GBufferFrame.getGBufferFrame(GBufferFrame.colorPass_GBuffer);
    this.godRayCompute.setSamplerTexture(
      `gBufferTexture`,
    ...
    this.godRayCompute.setSamplerTexture(
      `shadowMap`,
      shadowRenderer.depth2DArrayTexture
    );
  ...
  private createResource() {
    ...
    this.godRayTexture = new VirtualTexture(
      w,
      h,
  ...
  public onResize() {
    ...
    this.historyGodRayData.resizeBuffer(
      4 * this.godRayTexture.width * this.godRayTexture.height
    );
    this.godRayCompute.setStorageBuffer(
      "historyGodRayData",
  ...
  render(view: View3D, command: GPUCommandEncoder) {
    if (!this.godRayCompute) {
      ...
      this.godRayCompute.setUniformBuffer(
        "globalUniform",
