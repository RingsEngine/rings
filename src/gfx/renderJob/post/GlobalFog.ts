...
export class GlobalFog extends PostBase {
  ...
  private createCompute(view: View3D) {
    ...
    this.fogCompute.setSamplerTexture(
      "gBufferTexture",
    ...
    this.fogCompute.setStorageBuffer(
      `lightBuffer`,
  ...
  private createResource() {
    ...
    this.fogOpTexture = new VirtualTexture(
      w,
      h,
  ...
  render(view: View3D, command: GPUCommandEncoder) {
    if (!this.fogCompute) {
      ...
      this.fogCompute.setUniformBuffer(
        "globalUniform",
    ...
    this.fogCompute.setUniformFloat(
      "isSkyHDR",
      skyTexture.isHDRTexture ? 1 : 0
    );
