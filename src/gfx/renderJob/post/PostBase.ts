...
export class PostBase {
  ...
  protected createRTTexture(
    ...
    let rt = RTResourceMap.createRTTexture(
      name,
      rtWidth,
      rtHeight,
      format,
  ...
  protected createViewQuad(
    name: string,
    shaderName: string,
    outRtTexture: VirtualTexture,
    msaa: number = 0
  ) {
    let viewQuad = RTResourceMap.createViewQuad(
      name,
      "Quad_vert_wgsl",
      shaderName,
      outRtTexture,
  ...
  protected getOutTexture(): Texture {
    ...
    if (renderTargets.length > 0) {
      ...
      colorTexture = RTResourceMap.getTexture(
        RTResourceConfig.colorBufferTex_NAME
      );
    }
    return colorTexture;
  }
