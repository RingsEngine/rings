...
import {
  DDGIProbeRenderer,
  GIRenderCompleteEvent,
  GIRenderStartEvent,
} from "./DDGIProbeRenderer";

export let IrradianceDataReaderCompleteEvent: CEvent = new CEvent(
  "IrradianceDataReaderCompleteEvent"
);
/**
...
export class DDGIIrradianceGPUBufferReader extends CEventDispatcher {
  ...
  private async onProbeRenderComplete() {
    ...
    if (!this.readFlag) {
      ...
      await DDGIIrradianceGPUBufferReader.read(
        this.srcColorMap.getGPUTexture(),
        this.opColorBuffer,
        this.opColorArray
      );
      await DDGIIrradianceGPUBufferReader.read(
        this.srcDepthMap.getGPUTexture(),
        this.opDepthBuffer,
        this.opDepthArray
      );
      this.readFlag = false;
  ...
  private static async read(
    ...
    command.copyTextureToBuffer(
      { texture: srcTexture },
      { buffer: dstBuffer, bytesPerRow: srcTexture.width * 16 },
...
export let irradianceDataReader: DDGIIrradianceGPUBufferReader =
