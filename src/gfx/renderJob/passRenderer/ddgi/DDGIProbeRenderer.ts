...
export class DDGIProbeRenderer extends RendererBase {
  ...
  constructor(volume: DDGIIrradianceVolume) {
    ...
    let probeGBufferFrame = new ProbeGBufferFrame(
      this.sizeW,
      this.sizeH,
      false
    );
  ...
  public setInputTexture(textures: Texture[]) {
    ...
    this.bouncePass.setInputs([
      this.normalMap,
      this.colorMap,
      this.lightingPass.lightingTexture,
      this.irradianceColorMap,
    ]);
    this.irradianceComputePass.setTextures(
      [this.positionMap, this.normalMap, this.bouncePass.blendTexture],
      this.irradianceColorMap,
      this.irradianceDepthMap
    );
  }

  public setIrradianceData(
    ...
    if (
      width != this.irradianceColorMap.width ||
      height != this.irradianceColorMap.height
    ) {
  ...
  private renderSceneOnce(
    view: View3D,
    probeCamera: Camera3D,
    encoder: GPURenderPassEncoder,
    lights: ILight[]
  ) {
    this.volume.uploadBuffer();

    let collectInfo = EntityCollect.instance.getRenderNodes(
      view.scene,
      probeCamera
    );
    ...
    let drawMax = Math.min(
      Engine3D.setting.render.drawOpMax,
      collectInfo.opaqueList.length
    );
    ...
    drawMax = Math.min(
      Engine3D.setting.render.drawTrMax,
      collectInfo.transparentList.length
    );
  ...
  public render(view: View3D, occlusionSystem: OcclusionSystem) {
    ...
    if (
      EntityCollect.instance.state.giLightingChange ||
      probeBeRendered ||
      Engine3D.setting.gi.realTimeGI
    ) {
  ...
  private initIrradianceMap(volume: DDGIIrradianceVolume): void {
    ...
    this.irradianceDepthMap = new RenderTexture(
      setting.octRTMaxSize,
      setting.octRTMaxSize,
    ...
    this.irradianceColorMap = new RenderTexture(
      setting.octRTMaxSize,
      setting.octRTMaxSize,
      GPUTextureFormat.rgba16float,
      false,
