...
export class DDGIIrradianceComputePass {
  ...
  private initPipeline() {
    ...
    this.irradianceBuffer = new StorageGPUBuffer(
      pixelCount * 4,
      GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    );
    this.computeShader.setStorageBuffer(
      `irradianceBuffer`,
      this.irradianceBuffer
    );

    this.depthBuffer = new StorageGPUBuffer(
      pixelCount * 4,
      GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
    );
    this.computeShader.setStorageBuffer(`depthBuffer`, this.depthBuffer);

    this.depthRaysBuffer = new StorageGPUBuffer(
      4096 * 4 * 2 * 2 * 2,
      GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC
    );
    this.computeShader.setStorageBuffer(
      `depthRaysBuffer`,
      this.depthRaysBuffer
    );

    this.computeShader.setStorageBuffer(`probes`, this.volume.probesBuffer);
    this.computeShader.setUniformBuffer(
      `uniformData`,
  ...
  public setTextures(
    ...
    this.computeShader.setStorageTexture(
      `probeIrradianceMap`,
      this.probeIrradianceMap
    );
