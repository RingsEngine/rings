...
export class DDGIIrradianceVolume {
  ...
  private centerDirection: Vector3 = new Vector3(
    0,
    0,
  ...
  public updateOrientation(): Matrix4 {
    ...
    Matrix4.fromToRotation(
      this.centerDirection,
      this.arroundPositions[this.useRandomIndex],
  ...
  private createFramesBuffer(): void {
    if (!this.probesBufferData) {
      let size: number =
        this.setting.probeXCount *
        this.setting.probeYCount *
        this.setting.probeZCount;
      this.probesBufferData = new Float32Array(size * 4);
      this.probesBufferData.fill(-1);
      this.probesBuffer = new StorageGPUBuffer(
        size * 4,
  ...
  public uploadBuffer(): void {
    ...
    this.probesBuffer.setFloat32Array(
      "uniformFramesBuffer",
      this.probesBufferData
    );
  }
  ...
  public calcPosition(
    ...
    result.x =
      x * space - space * (setting.probeXCount - 1) * 0.5 + setting.offsetX;
    result.y =
      y * space - space * (setting.probeYCount - 1) * 0.5 + setting.offsetY;
    result.z =
      z * space - space * (setting.probeZCount - 1) * 0.5 + setting.offsetZ;
    return result;
  }
  ...
  private fillIrradianceData(): void {
    ...
    this.irradianceVolumeBuffer.setFloat(
      "orientationIndex",
      this.randomOrientation.index
    );
    this.irradianceVolumeBuffer.setFloat("hysteresis", setting.hysteresis);
    this.irradianceVolumeBuffer.setFloat(
      "OctRTSideSize",
      setting.octRTSideSize
    );
    ...
    this.irradianceVolumeBuffer.setFloat(
      "maxDistance",
      setting.probeSpace * 1.732
    ); //maxDistance = space * sqrt(3.0);
    this.irradianceVolumeBuffer.setFloat(
      "depthSharpness",
      setting.depthSharpness
    );
    this.irradianceVolumeBuffer.setFloat(
      "ProbeSourceTextureSize",
      setting.probeSourceTextureSize
    );
    this.irradianceVolumeBuffer.setFloat("ProbeSize", setting.probeSize);
    this.irradianceVolumeBuffer.setFloat(
      "bounceIntensity",
      setting.bounceIntensity
    );
    this.irradianceVolumeBuffer.setFloat(
      "probeRoughness",
      setting.probeRoughness
    );
    this.irradianceVolumeBuffer.setFloat("normalBias", setting.normalBias);
    this.irradianceVolumeBuffer.setFloat(
      "irradianceChebyshevBias",
      setting.irradianceChebyshevBias
    );
    this.irradianceVolumeBuffer.setFloat("rayNumber", setting.rayNumber);
    this.irradianceVolumeBuffer.setFloat(
      "irradianceDistanceBias",
      setting.irradianceDistanceBias
    );
    this.irradianceVolumeBuffer.setFloat(
      "indirectIntensity",
      setting.indirectIntensity
    );
    this.irradianceVolumeBuffer.setFloat("ddgiGamma", setting.ddgiGamma);
    this.irradianceVolumeBuffer.setFloat(
      "lerpHysteresis",
      setting.lerpHysteresis
    );
