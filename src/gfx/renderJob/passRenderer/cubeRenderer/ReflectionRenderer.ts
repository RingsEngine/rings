...
export class ReflectionRenderer extends RendererBase {
  ...
  constructor() {
    ...
    this.gBuffer = GBufferFrame.getGBufferFrame(
    ...
    this.preFilteredEnvironmentUniform = new UniformGPUBuffer(
      4 + this.probeCount * 4
    );
    ...
    this.preFilteredEnvironmentCompute = new ComputeShader(
      PreFilteredEnvironment_cs
    );
    this.preFilteredEnvironmentCompute.setSamplerTexture(
      "inputTex",
      this.gBuffer.getCompressGBufferTexture()
    );
    this.preFilteredEnvironmentCompute.setStorageTexture(
      "outputTexture",
      this.outTexture
    );
    this.preFilteredEnvironmentCompute.setUniformBuffer(
      "uniformData",
      this.preFilteredEnvironmentUniform
    );
  }
  ...
  public compute(view: View3D, occlusionSystem: OcclusionSystem): void {
    if (this.needUpdate) {
      ...
      this.preFilteredEnvironmentCompute.workerSizeX = Math.ceil(
        (this.probeSize * this.mipCount) / 16
      );
      this.preFilteredEnvironmentCompute.workerSizeY = Math.ceil(
        this.sizeH / 16
      );
      this.preFilteredEnvironmentCompute.workerSizeZ =
        this.mipCount * reflectionEntries.count;

      let command = GPUContext.beginCommandEncoder();
      GPUContext.computeCommand(command, [this.preFilteredEnvironmentCompute]);
  ...
  public render(
    ...
    for (let i = 0; i < reflections.length; i++) {
      ...
      if (
        (reflection.autoUpdate && (reflection.needUpdate || this.onChange)) ||
        Time.frame < 10
      ) {
  ...
  public drawNodes(
    ...
    if (viewRenderList) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (renderNode.hasMask(RendererMask.ReflectionDebug)) continue;
