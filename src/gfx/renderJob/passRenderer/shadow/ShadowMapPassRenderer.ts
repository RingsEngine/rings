...
export class ShadowMapPassRenderer extends RendererBase {
  ...
  setShadowMap(size: number, cascades: number) {
    ...
    this.depth2DArrayTexture = new Depth2DTextureArray(
      size,
      size,
  ...
  render(view: View3D, occlusionSystem: OcclusionSystem) {
    ...
    let shadowLightList =
      ShadowLightsCollect.getDirectShadowLightWhichScene(scene);
    ...
    for (let light of shadowLightList) {
      ...
      if (
        (dirLight.castShadow && dirLight.needUpdateShadow) ||
        this._forceUpdate ||
        ...
        if (camera.enableCSM && shadowIndex == 0) {
          for (let csmIndex = 0; csmIndex < cascades; csmIndex++) {
            ...
            this.poseShadowCamera(
              camera,
              dirLight.direction,
              csmChild.shadowCamera,
              extents,
              csmChild.bound.center
            );
            this.renderShadow(
              view,
              csmChild.shadowCamera,
            ...
            this.copyDepthTexture(
              this.rendererPassState.depthTexture,
              this.depth2DArrayTexture,
              csmIndex,
              shadowSize
            );
          }
  ...
  protected renderShadowBundleOp(
    view: View3D,
    shadowCamera: Camera3D,
    state: RendererPassState
  ) {
      ...
      entityBatchCollect.renderGroup.forEach((v) => {
        if (v.bundleMap.has(this._rendererType)) {
          ...
          let renderBundleEncoder = GPUContext.recordBundleEncoder(
            state.renderBundleEncoderDescriptor
          );
          this.recordShadowRenderBundleNode(
            view,
            shadowCamera,
            renderBundleEncoder,
            v.renderNodes
  ...
  protected renderShadowBundleTr(
    view: View3D,
    shadowCamera: Camera3D,
    state: RendererPassState
  ) {
  ...
  protected recordShadowRenderBundleNode(
    view: View3D,
    shadowCamera: Camera3D,
    encoder,
    nodes: RenderNode[],
