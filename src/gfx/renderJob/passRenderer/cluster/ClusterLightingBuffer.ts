...
export class ClusterLightingBuffer {
  ...
  constructor(numClusters: number, maxNumLightsPerCluster: number) {
    this.clusterBuffer = new ComputeGPUBuffer(
      numClusters * /*two vec4*/ 2 * /*vec4*/ 4
    );
    this.clustersUniformBuffer = new UniformGPUBuffer(10);
    this.clustersUniformBuffer.visibility =
      GPUShaderStage.FRAGMENT | GPUShaderStage.COMPUTE;

    this.lightAssignBuffer = new ComputeGPUBuffer(
      numClusters * maxNumLightsPerCluster
    );
    this.lightAssignBuffer.visibility =
      GPUShaderStage.FRAGMENT | GPUShaderStage.COMPUTE;
    this.assignTableBuffer = new ComputeGPUBuffer(numClusters * 4); // it has start and count
    this.assignTableBuffer.visibility =
      GPUShaderStage.FRAGMENT | GPUShaderStage.COMPUTE;
  }
  ...
  public update(
    ...
    this.clustersUniformBuffer.setFloat(
