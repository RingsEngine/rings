...
import {
  RendererMaskUtil,
  RendererMask,
} from "../renderJob/passRenderer/state/RendererMask";
...
export class PassGenerate {
  ...
  public static createShadowPass(renderNode: RenderNode, shader: Shader) {
    let use_skeleton = RendererMaskUtil.hasMask(
      renderNode.rendererMask,
      RendererMask.SkinnedMesh
    );
    let useTangent = renderNode.geometry.hasAttribute(
      VertexAttributeName.TANGENT
    );
    let useMorphTargets = renderNode.geometry.hasAttribute(
      GLTFType.MORPH_POSITION_PREFIX + "0"
    );
    let useMorphNormals = renderNode.geometry.hasAttribute(
      GLTFType.MORPH_NORMAL_PREFIX + "0"
    );
    ...
    for (let i = 0; i < colorPassList.length; i++) {
      ...
      if (!shadowPassList || shadowPassList.length < i + 1) {
        ...
        shadowPass.setUniform(
          `alphaCutoff`,
          colorPass.getUniform(`alphaCutoff`)
        );
        if (useTangent) {
      ...
      if (!castPointShadowPassList || castPointShadowPassList.length < i + 1) {
        ...
        castPointShadowPass.setTexture(
          `baseMap`,
          colorPass.getTexture(`baseMap`)
        );
        castPointShadowPass.setUniform(
          `alphaCutoff`,
          colorPass.getUniform(`alphaCutoff`)
        );
  ...
  static createReflectionPass(renderNode: RenderNode, shader: Shader) {
    ...
    for (let jj = 0; jj < colorPassList.length; jj++) {
      ...
      if (
        !colorSubPassList ||
        colorSubPassList.length == 0 ||
        colorSubPassList.length < jj
      ) {
