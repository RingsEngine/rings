...
export class ErpImage2CubeMap {
  public static convertRGBE2RGBA(
    image: VirtualTexture,
    data: Float32Array
  ): void {
    ...
    device.queue.writeBuffer(
      configBuffer,
      0,
      new Uint32Array([image.width, image.height, image.width, image.height])
    );
    ...
    computePass.dispatchWorkgroups(
      Math.floor(image.width / 8),
      Math.floor(image.height / 8)
    );
  ...
  public static makeTextureCube(
    ...
    device.queue.writeBuffer(
      ErpImage2CubeMap.configBuffer,
      0,
      new Uint32Array([image.width, image.height, dstSize, dstSize])
    );
