...
export class Line {
  ...
  public equals(l: Line): boolean {
    if (
      (this.start == l.start && this.end == l.end) ||
  ...
  public intersection(
    other: Line,
    pIntersectPoint: Vector3 = null
  ): LineClassification {
    var denom: number =
      (other.end.z - other.start.z) * (this.end.x - this.start.x) -
      (other.end.x - other.start.x) * (this.end.z - this.start.z);

    var u0: number =
      (other.end.x - other.start.x) * (this.start.z - other.start.z) -
      (other.end.z - other.start.z) * (this.start.x - other.start.x);

    var u1: number =
      (other.start.x - this.start.x) * (this.end.z - this.start.z) -
      (other.start.z - this.start.z) * (this.end.x - this.start.x);
  ...
  public static squreDistanceSegmentToSegment(
    lineA: Line,
    lineB: Line,
    mat?: Matrix4
  ) {
  ...
  public isNear(ray: Ray, maxDistance: number = 0, mat?: Matrix4): boolean {
    ...
    if (mat) {
      mat.perspectiveMultiplyPoint3(
        Line.cacluteLine1.start,
        Line.cacluteLine1.start
      );
      mat.perspectiveMultiplyPoint3(
        Line.cacluteLine1.end,
        Line.cacluteLine1.end
      );
    }

    let dis = Line.squreDistanceSegmentToSegment(
      Line.cacluteLine0,
      Line.cacluteLine1,
      mat
    );
