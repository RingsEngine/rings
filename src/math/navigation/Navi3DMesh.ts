...
export class Navi3DMesh {
  ...
  constructor(
    ...
    triangleIndexList: Array<Array<number>>
  ) {
  ...
  public findPath(
    ...
    if (success) {
      ...
      success = this._nav3dFunnel.searchPath(
        startPt,
        endPt,
        this._triangleList,
        aiRadius
  ...
  private initEdgesAndTriangles(triangleIndexList: Array<Array<number>>): void {
    ...
    for (var i: number = 0; i < count; i++) {
      ...
      if (edge0 == null || edge1 == null || edge2 == null) continue;
  ...
  private tryCreateEdge(pointAId: number, pointBId: number): Navi3DEdge {
    ...
    var edge: Navi3DEdge = this._edgesDict.getValueByKey(
      pointAId + "_" + pointBId
    );
    if (edge == null) {
      edge = new Navi3DEdge(
        this._nav3dPoints[pointAId],
        this._nav3dPoints[pointBId]
      );
