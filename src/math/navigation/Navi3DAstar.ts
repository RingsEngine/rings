...
export class Navi3DAstar {
  ...
  public findPath(
    navMesh: Navi3DMesh,
    startTriangle: Navi3DTriangle,
    endTriangle: Navi3DTriangle
  ): boolean {
    this._findIndex++;
  ...
  private search(): boolean {
    ...
    while (node != this._endNode) {
      neibours = node.getNeibourTriangles(
        neibours,
        Navi3DMaskType.WalkAble,
        Navi3DMaskType.WalkAble
      );
      for (test of neibours) {
        if (test.closeId == this._findIndex) continue;
        if (test == node || !test.walkAble) {
          continue;
        }
        var g: number =
          node.gg + Navi3DPoint.calcDistance(test, node) * test.costMultiplier;
