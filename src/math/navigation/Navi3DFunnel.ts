...
export class Navi3DFunnel {
  ...
  public searchPath(
    ...
    //
    //起点终点判断
    if (!this.searchEnable(startPt, endPt, triangleList)) return false;
  ...
  private searchEnable(
    ...
    if (startPt == null || endPt == null || triangleList == null) return false;

    if (
      triangleList[0].plane.classifyPoint(startPt, Navi3DConst.EPSILON) !=
      PlaneClassification.INTERSECT
    ) {
      return false;
    }
    if (
      triangleList[triangleList.length - 1].plane.classifyPoint(
        endPt,
        Navi3DConst.EPSILON
      ) != PlaneClassification.INTERSECT
    ) {
  ...
  private search(
    ...
    for (i = 0; i < crossedEdgeCount; i++) {
      ...
      this._tempSamePlaneList.push(
        plane.classifyPoint(pt, Navi3DConst.EPSILON) ==
          PlaneClassification.INTERSECT
      );
    }
    ...
    for (i = 0; i < crossedEdgeCount; i++) {
      ...
      continuePass = this._router.passEdge(
        curEdge,
        this._tempPublicEdgeList[i + 1],
        pt,
        lastEdge
      );
      if (!continuePass) {
      ...
      if (curEdge.crossPoint) {
        ...
        if (fatPoint) {
          ...
          if (toPoint == fromPoint) {
            ...
            Navi3DFunnel.CROSS_TEST_DIRECTION.setTo(
              toPoint.x - fromPoint.x,
              0,
              toPoint.z - fromPoint.z
            );
            crossPoint = this._router.calcCrossEdge(
              curEdge,
              fromPoint,
              Navi3DFunnel.CROSS_TEST_DIRECTION
            );
          }
  ...
  private optimusByRadius(): void {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (checkEnable) {
        ...
        checkEnable = this._router.hasCrossPoint(
          centerEdge.pointA,
          centerEdge.pointB,
          pt2,
          Navi3DFunnel.CROSS_TEST_DIRECTION
        ...
        if (checkEnable) {
          optimusPoint = this._router.calcCrossPointOut(
            edgePt1,
            pt1,
            pt2,
            Navi3DFunnel.CROSS_TEST_DIRECTION
