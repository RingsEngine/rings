...
export class PolynomialCurve {
  ...
  public generateIntegrationCache(curve: PolynomialCurve) {
    ...
    for (let i = 1; i < curve.segmentCount; i++) {
      ...
      curve.integrationCache[i] =
        curve.integrationCache[i - 1] +
        Polynomial.EvalSegment(time, coeff) * time;
  ...
  public generateDoubleIntegrationCache(curve: PolynomialCurve) {
    ...
    for (let i = 0; i < curve.segmentCount; i++) {
      ...
      sum +=
        Polynomial.EvalSegment(time, curve.segments[i].coeff) * time * time +
        curve.integrationCache[i] * time;
  ...
  public evaluateDoubleIntegrated(t) {
    ...
    for (let i = 0; i < this.segmentCount; i++) {
      if (t <= this.times[i]) {
        ...
        return (
          this.doubleIntegrationCache[i] +
          this.integrationCache[i] * time +
          Polynomial.EvalSegment(time, this.segments[i].coeff) * time * time
        );
      }
      prevTimeValue = this.times[i];
    }
  ...
  public evaluateIntegrated(t) {
    ...
    for (let i = 0; i < this.segmentCount; i++) {
      if (t <= this.times[i]) {
        let time = t - prevTimeValue;
        return (
          this.integrationCache[i] +
          Polynomial.EvalSegment(time, this.segments[i].coeff) * time
        );
      }
      prevTimeValue = this.times[i];
    }
