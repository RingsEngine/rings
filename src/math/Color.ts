export class Color {
   ...
   * @internal
   */
  private static NON_HEX_CHARS = new RegExp(
    `[^#${Color.HEX_CHARACTERS}]`,
    "gi"
  );
  /**
   * @internal
   */
  private static VALID_HEX_SIZE = new RegExp(
    `^${Color.MATCH_3OR4_HEX}$|^${Color.MATCH_6OR8_HEX}$`,
    "i"
  );
  ...
  public setHex(hex: string) {
    if (
      typeof hex !== "string" ||
      Color.NON_HEX_CHARS.test(hex) ||
      !Color.VALID_HEX_SIZE.test(hex)
    ) {
