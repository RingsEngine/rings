...
export class TilesRenderer {
  ...
  public async loadTileSet(rootPath: string, file: string) {
    ...
    const upAxis =
      (this._tileSet.asset && this._tileSet.asset.gltfUpAxis) || "y";
    ...
    for (let item of this._tileSet.root.children) {
      ...
      for (let uriPath of uriList) {
        ...
        if (url.endsWith(".glb")) {
          tileObject3D = (await Engine3D.res.loadGltf(
            url,
            functions
          )) as Object3D;
          ...
          tileObject3D = (await Engine3D.res.loadI3DM(
            url,
            functions,
            adjustmentTransform
          )) as Object3D;
        } else if (url.endsWith(".b3dm")) {
          tileObject3D = (await Engine3D.res.loadB3DM(
            url,
            functions,
            adjustmentTransform
          )) as Object3D;
