...
export class GLTFSubParserConverter {
  ...
  public async convertNodeToObject3D(
    nodeInfo: GLTF_Node,
    parentNode
  ): Promise<Object3D> {
  ...
  private convertLight(nodeInfo: GLTF_Node, node: Object3D) {
    switch (nodeInfo.light.type) {
      case `directional`:
        ...
        directLight.lightColor = nodeInfo.light.color
          ? new Color(
              nodeInfo.light.color[0],
              nodeInfo.light.color[1],
              nodeInfo.light.color[2]
            )
          : new Color(1, 1, 1, 1);
        directLight.debug();
        break;
      case `point`:
        if (this._testCount > 0) {
          ...
          point.intensity = nodeInfo.light.intensity
            ? nodeInfo.light.intensity * 8.0 * 2
            : 1.0;
          point.radius = 8.0;
          point.at = 2;
          point.range = nodeInfo.light.range ? nodeInfo.light.range : 8;
          point.lightColor = nodeInfo.light.color
            ? new Color(
                nodeInfo.light.color[0],
                nodeInfo.light.color[1],
                nodeInfo.light.color[2]
      ...
      case `spot`:
        ...
        spot.outerAngle =
          nodeInfo.light.spot.outerConeAngle * RADIANS_TO_DEGREES;
        spot.lightColor = nodeInfo.light.color
          ? new Color(
              nodeInfo.light.color[0],
              nodeInfo.light.color[1],
              nodeInfo.light.color[2]
    ...
    for (let i = 0; i < nodeInfo.primitives.length; i++) {
      ...
      if (md && this.gltf.resources[materialKey]) {
        ...
        if (gltfMat) {
          ...
          let physicMaterial = (newMat = this.applyMaterialExtensions(
            gltfMat,
            newMat
          ));
      ...
      if (
        attribArrays[`indices`].data &&
        attribArrays[`indices`].data.length > 3
      ) {
        ...
        if (
          ...
          geometry ||= this.createGeometryBase(
            modelName,
            attribArrays,
            primitive,
            nodeInfo.skin
  ...
  private createGeometryBase(
    ...
    if ("indices" in attribArrays) {
      ...
      if (bigIndices) {
        attribArrays[`indices`].data = new Uint32Array(
          attribArrays[`indices`].data
        );
      } else {
        attribArrays[`indices`].data = new Uint16Array(
          attribArrays[`indices`].data
        );
      }
    }
    ...
    if (primitive.morphTargetsRelative) {
      ...
      for (let i = 0; i < blendShapeData.blendCount; i++) {
        ...
        propertyData.blendPositionList =
          attribArrays[GLTFType.MORPH_POSITION_PREFIX + i].data;
        propertyData.blendNormalList =
          attribArrays[GLTFType.MORPH_NORMAL_PREFIX + i].data;
