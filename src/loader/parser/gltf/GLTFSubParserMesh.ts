...
export class GLTFSubParserMesh {
  ...
  public async parse(meshId) {
    ...
    if (!mesh) return this.errorMiss("mesh", meshId);

    if (mesh.isParsed) return mesh.dprimitives;
    ...
    for (let i = 0; i < primitives.length; i++) {
      ...
      if (extensions && extensions.KHR_draco_mesh_compression) {
        overlayAccessors = await KHR_draco_mesh_compression.apply(
          this.subParser,
          primitive
        );
      }
      ...
      for (const attribute in attributes) {
        const accessor = overlayAccessors
          ? overlayAccessors[attribute]
          : this.parseAccessor(attributes[attribute]);
      ...
      if (vertexColor)
        dprimitive.defines.push(GLTFParser.getVertexColorDefine(vertexColor));

      if (indices !== undefined) {
        const accessor = overlayAccessors
          ? overlayAccessors["indices"]
          : this.parseAccessor(indices);
      ...
      if (targets) {
        dprimitive.defines.push(
          GLTFParser.getMorphTargetsDefine(targets.length)
        );
        ...
        for (let j = 0; j < targets.length; j++) {
          ...
          Object.keys(target).forEach((attribute) => {
            ...
            if (accessor) {
              ...
              if (!attribName)
                console.error(
                  `glTF has unsupported morph target attribute ${attribute}`
                );
