...
export class GLTFSubParser {
  ...
  public async parse(initUrl: string, gltf, sceneId) {
    ...
    if (version !== "2.0") {
      console.error(
        `GLTFParser only support glTF 2.0 for now! Received glTF version: ${this.version}`
      );
      return false;
    }
  ...
  private async parseNode(nodeId) {
    ...
    if (!node) return this.errorMiss("node", nodeId);

    if (node.isParsed) return node.dnode;
    ...
    if (node.camera !== undefined) dnode.camera = this.parseCamera(node.camera);

    if (node.mesh !== undefined)
      dnode.primitives = await this.parseMesh(node.mesh);

    if (node.extensions !== undefined) this.applyNodeExtensions(node, dnode);
  ...
  public parseSkeletonAnimation(
    avatarData: PrefabAvatarData,
    animation
  ...
  public parseAccessor(accessorId) {
    ...
    let typedArray = this.getTypedArrayFromArrayBuffer(
      buffer,
      byteStride,
    ...
    if (accessor.sparse) {
      ...
      const indicesArrayType = getTypedArrayTypeFromGLType(
        indices.componentType
      );
      const indicesBuffer = this.parseBufferView(indices.bufferView);
      const indicesArray = this.getTypedArrayFromArrayBuffer(
        indicesBuffer,
      ...
      const valuesArray = this.getTypedArrayFromArrayBuffer(
        valuesBuffer,
        valuesBufferView.byteStride,
        valuesByteOffset,
        arrayType,
        numComponents,
        count
      ...
      for (let i = 0; i < indicesArray.length; i++)
        typedArray.set(
          valuesArray.slice(
            i * numComponents,
            i * numComponents + numComponents
          ),
          indicesArray[i] * numComponents
        );
    }
  ...
  private getTypedArrayFromArrayBuffer(
    ...
    if (byteStride && componentsBytes !== byteStride) {
      ...
      for (let i = 0; i < count; i++) {
        const componentVals = new arrayType(
          buffer,
          byteOffset + i * byteStride,
          numComponents
        ); // eslint-disable-line
    ...
    if (buffer.uri.substring(0, 5) !== "data:") {
      ...
      if (arrayBuffer)
        if (arrayBuffer.byteLength === buffer.byteLength) {
          ...
          console.error(
            `load gltf resource "${uri}" at buffers[${bufferId} failed, ArrayBuffer.byteLength not equals buffer's byteLength]`
          );
      else
        console.error(
          `load gltf resource "${uri}" at buffers[${bufferId}] failed`
        );
    } else {
      const base64Idx =
        buffer.uri.indexOf(this._BASE64_MARKER) + this._BASE64_MARKER.length;
