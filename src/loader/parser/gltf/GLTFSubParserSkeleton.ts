...
export class GLTFSubParserSkeleton {
  ...
  public parseSkeletonAnimation(
    ...
    for (let channel of animation.channels) {
      ...
      switch (property) {
        case "scale":
          {
            let animationCurveT = new AnimationCurveT(
              outputAccessor.numComponents
            );
  ...
  public parseSkeletonAnimationOld(skeleton: Skeleton, animation) {
    let count = 0;
    let inputAccessor = this.subParser.parseAccessor(
    ...
    for (let channel of animation.channels) {
      ...
      if (!joint) {
        ...
        switch (property) {
          case "scale":
            if (
              numFrame * outputAccessor.numComponents ==
              outputAccessor.data.length
            ) {
              for (var nFrame: number = 0; nFrame < numFrame; nFrame++) {
          ...
          case "rotation":
            if (
              ...
              for (var nFrame: number = 0; nFrame < numFrame; nFrame++) {
                var srcOffset = nFrame * outputAccessor.numComponents;
                var dstOffset =
                  skeletonPoseLength * nFrame + 12 * joint.index + 4;
                bufferData[dstOffset + 0] = outputAccessor.data[srcOffset + 0]; // x
                bufferData[dstOffset + 1] = outputAccessor.data[srcOffset + 1]; // y
                bufferData[dstOffset + 2] = outputAccessor.data[srcOffset + 2]; // z
          ...
          case "translation":
            if (
              ...
              for (var nFrame: number = 0; nFrame < numFrame; nFrame++) {
                var srcOffset = nFrame * outputAccessor.numComponents;
                var dstOffset =
                  skeletonPoseLength * nFrame + 12 * joint.index + 8;
                bufferData[dstOffset + 0] = outputAccessor.data[srcOffset + 0]; // x
                bufferData[dstOffset + 1] = outputAccessor.data[srcOffset + 1]; // y
                bufferData[dstOffset + 2] = outputAccessor.data[srcOffset + 2]; // z
                bufferData[dstOffset + 3] =
                  inputAccessor.data[nFrame * inputAccessor.numComponents];
              }
            } else if (inputAccessor.data.length == 2) {
              let time = 0;
              let t0 = inputAccessor.data[0];
    ...
    let animationClip = new SkeletonAnimationClip(
      animation.name,
      skeleton,
      numFrame,
      bufferData
