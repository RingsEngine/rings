...
export class TranslationControlComponents extends TransformControllerBaseComponent {
  public init(param?: any): void {
    ...
    this.mAxisCollider[TransformAxisEnum.XY] =
      planeXY.getComponent(ColliderComponent);
    this.mAxisCollider[TransformAxisEnum.XZ] =
      planeXZ.getComponent(ColliderComponent);
    this.mAxisCollider[TransformAxisEnum.YZ] =
      planeYZ.getComponent(ColliderComponent);
  }
  ...
  protected applyLocalTransform(
    ...
    if (
      !(
        this.currentAxis == TransformAxisEnum.X ||
        this.currentAxis == TransformAxisEnum.XY ||
        this.currentAxis == TransformAxisEnum.XZ
      )
    ) {
      Vector3.HELP_0.x = 0;
    }
    if (
      !(
        this.currentAxis == TransformAxisEnum.Y ||
        this.currentAxis == TransformAxisEnum.XY ||
        this.currentAxis == TransformAxisEnum.YZ
      )
    ) {
      Vector3.HELP_0.y = 0;
    }
    if (
      !(
        this.currentAxis == TransformAxisEnum.Z ||
        this.currentAxis == TransformAxisEnum.XZ ||
        this.currentAxis == TransformAxisEnum.YZ
      )
    ) {
    ...
    this.mX.transform.worldMatrix.transformVector(
      Vector3.HELP_0,
      Vector3.HELP_1
    );
