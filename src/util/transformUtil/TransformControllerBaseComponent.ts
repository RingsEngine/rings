...
export class TransformControllerBaseComponent extends ComponentBase {
  ...
  public init(param?: any): void {
    ...
    this.mAxisCollider[TransformAxisEnum.X] =
      axisX.getComponent(ColliderComponent);
    this.mAxisCollider[TransformAxisEnum.Y] =
      axisY.getComponent(ColliderComponent);
    this.mAxisCollider[TransformAxisEnum.Z] =
      axisZ.getComponent(ColliderComponent);
  }
  ...
  public reset() {
    switch (this.transformSpaceMode) {
      case TransformSpaceMode.Local:
        {
          let targetInvWorldMatrix = Matrix4.help_matrix_0.copyFrom(
            this.mX.transform.worldMatrix
          );
  ...
  protected pickAxis(): {
    ...
    let ray = camera.screenPointToRay(
    ...
    for (let i = 0; i <= TransformAxisEnum.MAX; i++) {
      ...
      if (intersect) {
        if (
          !lastResult ||
          lastResult.distance > intersect.distance ||
          i == TransformAxisEnum.XYZ
        ) {
          lastResult = {
            axis: i,
  ...
  public onMouseDown(e: PointerEvent3D): void {
    ...
    const scene3D = this.object3D.transform.scene3D;
    const camera = scene3D.view.camera;
    let screenPoint = camera.worldToScreenPoint(
      this.mX.transform.worldPosition
    );
    let pos = camera.screenPointToWorld(
      Engine3D.inputSystem.mouseX,
      Engine3D.inputSystem.mouseY,
      screenPoint.z
    );
    ...
    if (this.currentAxis == TransformAxisEnum.NONE) {
      ...
      if (this.lastMoveObj && "baseColor" in mat) {
        ...
        if (this.lastMoveAxis == TransformAxisEnum.XYZ) {
          this.mAxis[TransformAxisEnum.X]
            .getComponent(MeshRenderer)
            .material.setUniformColor(
              "baseColor",
              this.mAxisColor[TransformAxisEnum.X]
            );
          this.mAxis[TransformAxisEnum.Y]
            .getComponent(MeshRenderer)
            .material.setUniformColor(
              "baseColor",
              this.mAxisColor[TransformAxisEnum.Y]
            );
          this.mAxis[TransformAxisEnum.Z]
            .getComponent(MeshRenderer)
            .material.setUniformColor(
              "baseColor",
              this.mAxisColor[TransformAxisEnum.Z]
            );
        }
      ...
      if (this.lastMoveAxis == TransformAxisEnum.XYZ) {
        this.mAxis[TransformAxisEnum.X]
          .getComponent(MeshRenderer)
          .material.setUniformColor("baseColor", new Color(1, 1, 1));
        this.mAxis[TransformAxisEnum.Y]
          .getComponent(MeshRenderer)
          .material.setUniformColor("baseColor", new Color(1, 1, 1));
        this.mAxis[TransformAxisEnum.Z]
  ...
  public onUpdate(view?: View3D) {
    let distance = Vector3.distance(
      view.camera.transform.worldPosition,
      this.object3D.transform.worldPosition
    );
    let scale = distance / 100.0;
    this.mContainer.scaleX =
      this.mContainer.scaleY =
      this.mContainer.scaleZ =
        scale;
    if (this.mX) {
      const pos = this.mX.transform.worldPosition;
      this.object3D.x = pos.x;
  ...
  protected applyLocalTransform(
    currentAxis: TransformAxisEnum,
    offset: Vector3,
    distance: number
  ) {
  ...
  protected applyGlobalTransform(
    currentAxis: TransformAxisEnum,
    offset: Vector3,
    distance: number
  ) {
  ...
  protected createAxis(axis: TransformAxisEnum): Object3D {
    ...
    boxShape.setFromCenterAndSize(
      new Vector3(0, 0, 0),
      new Vector3(w + 1, h + 1, d + 1)
    );
