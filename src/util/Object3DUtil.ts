...
export class Object3DUtil {
  ...
  private static initHeap() {
    if (!this.boxGeo) this.boxGeo = new BoxGeometry();
    if (!this.planeGeo)
      this.planeGeo = new PlaneGeometry(1, 1, 1, 1, Vector3.UP);
    if (!this.sphere) this.sphere = new SphereGeometry(1, 35, 35);
    if (!this.material) {
