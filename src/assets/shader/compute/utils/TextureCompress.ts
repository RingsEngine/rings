...
export let textureCompress = (
  ...
  for (let i = 0; i < sourceTexture.length; i++) {
    ...
    bindInputTexture += /* wgsl */ `
            @group(0) @binding(${
              (begin + i) * 2 + 0
            }) var ${inTextureName} : texture_2d<f32>;\n
            @group(0) @binding(${
              (begin + i) * 2 + 1
            }) var ${inTextureSamplerName} : sampler;
        `;

    preG += GaussBlur(
      `GaussBlur_${inTextureName}_1`,
      inTextureName,
      inTextureSamplerName
    );
    preG += GaussBlur(
      `GaussBlur_${inTextureName}_0`,
      inTextureName,
      inTextureSamplerName
    );
  }
  ...
  for (let i = 0; i < destTexture.length; i++) {
    bindOutputTexture += /* wgsl */ `@group(0) @binding(${
      (begin + i) * 2 + 0
    }) var dest${i}Map : texture_storage_2d<rgba16float, write>;\n`;
  }

  let loadMainColor = "";
  if (colorMap) {
    begin += destTexture.length;
    bindInputTexture += /* wgsl */ `@group(0) @binding(${
      begin * 2 + 0
  ...
  let copyTexture = copyToTexture(
    sourceTexture.length,
    colorMap ? true : false
  );
...
let upSample = (
  funName: string,
  outeTextureName: string,
  combineMainColor: boolean
) => {
  let code = /* wgsl */ `
