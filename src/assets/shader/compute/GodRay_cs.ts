...
export let GodRay_cs: string = /*wgsl*/ `
    ...
    @group(1) @binding(0) var<storage,read> lightBuffer: array<LightData>;
    @group(1) @binding(1) var<storage, read> models : Uniforms;
    ...
    fn directShadowMaping(P:vec3<f32>, N:vec2<f32>, shadowBias: f32) {
    ...
    fn directShadowMapingIndex(light:LightData, matrix:mat4x4<f32>, P:vec3<f32>, N:vec3<f32>, depthTexIndex:i32, shadowBias:f32) -> vec2<f32> {
        ...
        if(varying_shadowUV.x <= 1.0 && varying_shadowUV.x >= 0.0 && varying_shadowUV.y <= 1.0 && varying_shadowUV.y >= 0.0 && shadowPosTmp.z <= 1.0 && shadowPosTmp.z >= 0.0){
            isOutSideArea = 0.0;
    ...
    fn rayMarch() -> f32{
        ...
        if(godRayUniform.blendColor > 0.5){
            ...
            for(var i:f32 = 1.0; i< rayMarchCount; i += 1.0){
                ...
                var shadowVisibility = shadowStruct.directShadowVisibility;
