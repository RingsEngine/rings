...
import {
  RendererMask,
  RendererMaskUtil,
} from "../../gfx/renderJob/passRenderer/state/RendererMask";
...
export class RenderNode extends ComponentBase {
  ...
  protected castNeedPass() {
    ...
    // add if alpha == 1
    let ignoreDepthPass = RendererMaskUtil.hasMask(
      this.rendererMask,
      RendererMask.IgnoreDepthPass
    );
  ...
  public renderPass(
    ...
    const nCount = Math.max(
      renderNode.materials.length,
      renderNode._geometry.subGeometries.length
    );

    for (let i = 0; i < nCount; i++) {
      const material =
        i >= renderNode.materials.length
          ? renderNode.materials[0]
          : renderNode.materials[i];
      if (!material || !material.enable) continue;

      let passes = material.getPass(passType);

      if (!passes || passes.length == 0) continue;

      GPUContext.bindGeometryBuffer(
        renderContext.encoder,
        renderNode._geometry
      );
      ProfilerUtil.viewCount_vertex(
        view,
        PassType[passType],
        renderNode._geometry.vertexCount
      );
      ...
      for (let j = 0; j < passes.length; j++) {
        ...
        if (renderShader.pipeline) {
          ...
          let noneShare = GPUContext.bindPipeline(
            renderContext.encoder,
            renderShader
          );
          if (noneShare) {
          ...
          let subGeometry =
            i >= renderNode._geometry.subGeometries.length
              ? renderNode._geometry.subGeometries[0]
              : renderNode._geometry.subGeometries[i];
          let lodInfos = subGeometry.lodLevels;
          let lodInfo = lodInfos[renderNode.lodLevel];

          if (renderNode.instanceCount > 0) {
            ProfilerUtil.viewCount_instance(
              view,
              PassType[passType],
              renderNode.instanceCount
            );
            ProfilerUtil.viewCount_indices(
              view,
              PassType[passType],
              lodInfo.indexCount
            );
            ProfilerUtil.viewCount_tri(
              view,
              PassType[passType],
              (lodInfo.indexCount / 3) * renderNode.instanceCount
            );
  ...
  public renderPass2(
    ...
    for (let i = 0; i < this.materials.length; i++) {
      ...
      if (!material.castShadow && passType == PassType.SHADOW) continue;
      let passes = material.getPass(passType);
    ...
    for (let j = 0; j < node.materials.length; j++) {
      ...
      if (passes) {
        for (let i = 0; i < passes.length; i++) {
          ...
          if (renderShader.shaderState.splitTexture) {
            let splitTexture = RTResourceMap.CreateSplitTexture(
              node.instanceID
            );
          ...
          let reflectionEntries = GlobalBindGroup.getReflectionEntries(
            view.scene
          );
          if (
            !renderShader.reflectionMap &&
            reflectionEntries &&
            reflectionEntries.reflectionMap
          ) {
            renderShader.setTexture(
              `reflectionMap`,
              reflectionEntries.reflectionMap
            );
            renderShader.setStorageBuffer(
              `reflectionBuffer`,
              reflectionEntries.storageGPUBuffer
            );
          }

          if (renderShader.pipeline) {
            renderShader.apply(node._geometry, renderPassState, () =>
              node.noticeShaderChange()
            );
            continue;
          }
          ...
          let shadowRenderer =
            Engine3D.getRenderJob(view).shadowMapPassRenderer;
          ...
          let pointShadowRenderer =
            Engine3D.getRenderJob(view).pointLightShadowRenderer;
          if (pointShadowRenderer && pointShadowRenderer.cubeArrayTexture) {
            renderShader.setTexture(
              `pointShadowMap`,
              pointShadowRenderer.cubeArrayTexture
            );
          }
          ...
          if (
            renderPassState.irradianceBuffer &&
            renderPassState.irradianceBuffer.length > 0
          ) {
            renderShader.setTexture(
              `irradianceMap`,
              renderPassState.irradianceBuffer[0]
            );
            renderShader.setTexture(
              `irradianceDepthMap`,
              renderPassState.irradianceBuffer[1]
            );
          }
          ...
          if (lightUniformEntries) {
            renderShader.setStorageBuffer(
              `lightBuffer`,
              lightUniformEntries.storageGPUBuffer
            );
            if (lightUniformEntries.irradianceVolume) {
              renderShader.setUniformBuffer(
                `irradianceData`,
                lightUniformEntries.irradianceVolume.irradianceVolumeBuffer
              );
            }
          }
          ...
          if (clusterLightingBuffer) {
            renderShader.setStorageBuffer(
              `clustersUniform`,
              clusterLightingBuffer.clustersUniformBuffer
            );
            renderShader.setStorageBuffer(
              `lightAssignBuffer`,
              clusterLightingBuffer.lightAssignBuffer
            );
            renderShader.setStorageBuffer(
              `assignTable`,
              clusterLightingBuffer.assignTableBuffer
            );
            renderShader.setStorageBuffer(
              `clusterBuffer`,
              clusterLightingBuffer.clusterBuffer
            );
          }
