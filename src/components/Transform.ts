...
export class Transform extends ComponentBase {
  ...
  public set parent(value: Transform) {
    ...
    WasmMatrix.setParent(
      this.index,
      value ? value.worldMatrix.index : -1,
      this.depthOrder
    );
  ...
  constructor() {
    ...
    WasmMatrix.setRotation(
      this.index,
      this._localRot.x,
      this._localRot.y,
      this._localRot.z
    );
    WasmMatrix.setTranslate(
      this.index,
      this._localPos.x,
      this._localPos.y,
      this._localPos.z
  ...
  public set localRotation(v: Vector3) {
    if (this.onRotationChange) {
      if (
        this._localRot.x != v.x ||
        this._localRot.y != v.y ||
        this._localRot.z != v.z
      ) {
