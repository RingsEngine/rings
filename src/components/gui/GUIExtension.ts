...
export function makeGUISprite(source: GUITexture, id: string, data: any) {
  ...
  sprite.uvRec.set(
    sprite.uvRec.x * wScale,
    sprite.uvRec.y * hScale,
    sprite.uvRec.z * wScale,
    sprite.uvRec.w * hScale
  );
  let tiny = 0.1;
  if (
    data.border.x <= tiny &&
    ...
    sprite.uvBorder.z =
      data.border.z -
      (data.size.x - data.textureRect.z - data.textureRectOffset.x);
    sprite.uvBorder.w =
      data.border.w -
      (data.size.y - data.textureRect.w - data.textureRectOffset.y);
