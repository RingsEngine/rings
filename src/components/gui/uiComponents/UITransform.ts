...
export class UITransform extends ComponentBase {
  ...
  public init(param?: any): void {
    ...
    this.transform.eventDispatcher.addEventListener(
      this.transform.eventLocalChange.type,
  ...
  protected onUITransformVisible(global: boolean): void {
    ...
    if (newGlobalVisible != this._globalVisible) {
      ...
      this.object3D.components.forEach((v, k) => {
        ...
        if (!ui.onUITransformVisible) return;
        if (ui == this) {
  ...
  public set onChange(value: boolean) {
    if (this._onChange != value) {
      ...
      if (value) {
        ...
        let components = this.object3D.getComponents(
          UITransform,
          this._tempTransforms,
          true
        );
  ...
  public matrix(): Matrix3 {
    ...
    if (this.parent) {
      mtx.updateScaleAndRotation(
        this.object3D.scaleX,
        this.object3D.scaleY,
        rot,
        rot
      );
    ...
    if (this.pivotX != 0.5 || this.pivotY != 0.5) {
      ...
      mtx.mul(
        help_matrix3.setTo(
          ...
          0,
          1,
          -(this.pivotX - 0.5) * this.width,
          -(this.pivotY - 0.5) * this.height
        )
      );
    }
    return mtx;
  ...
  public getWorldMatrix(): Matrix3 {
    ...
    if (!this._onChange) return worldMtx;
