...
export class UIPanel extends UIImage {
  ...
  init(param?: any) {
    super.init(param);
    this._uiTransform.resize(
      webGPUContext.canvas.width,
      webGPUContext.canvas.height
    );
    this.create(this.space);
    this.visible = false;
  }

  public updateDrawCallSegment(
    index: number,
    indexStart: number,
    indexCount: number
  ) {
    ...
    let newMaterial: GUIMaterial = this._uiRenderer.materials[
      index
    ] as GUIMaterial;
    if (!newMaterial) {
  ...
  private create(space: GUISpace) {
    this._maxCount =
      this.space == GUISpace.World
        ? GUIConfig.quadMaxCountForWorld
        : GUIConfig.quadMaxCountForView;
    this._uiRenderer = this.object3D.addComponent(GUIRenderer);
    this._geometry = this._uiRenderer.geometry = new GUIGeometry(
      this._maxCount
    ).create();
    ...
    this.object3D.bound = new BoundingBox(
      new Vector3(),
      new Vector3(1, 1, 1).multiplyScalar(Number.MAX_VALUE * 0.1)
    );
  }
  ...
  public set billboard(type: BillboardType) {
    ...
    if (
      type == BillboardType.BillboardXYZ ||
      type == BillboardType.BillboardY
    ) {
  ...
  private rebuildGUIMesh(view: View3D) {
    ...
    let renderStart = panel["isViewPanel"]
      ? GUIConfig.SortOrderStartView
      : GUIConfig.SortOrderStartWorld;
    panel._uiRenderer.renderOrder =
      canvasIndex * GUIConfig.SortOrderCanvasSpan +
      renderStart +
      panel.panelOrder;
    ...
    for (let item of panel["_uiRenderer"].materials) {
      ...
      material.setScreenSize(
        webGPUContext.canvas.clientWidth,
        webGPUContext.canvas.clientHeight
      );
      material.setScissorEnable(panel.scissorEnable);
      if (panel.scissorEnable) {
        let maskQuad = panel.mainQuads[0];
        material.setScissorRect(
          maskQuad.left,
          maskQuad.bottom,
          maskQuad.right,
          maskQuad.top
        );
        material.setScissorCorner(
          panel.scissorCornerRadius,
          panel.scissorFadeOutSize
        );
      }
    }
