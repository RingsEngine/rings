...
import {
  PickGUIEvent3D,
  PointerEvent3D,
} from "../../event/eventConst/PointerEvent3D";
...
import {
  ...
  UIInteractiveStyle,
  GUIHitInfo,
} from "./uiComponents/IUIInteractive";
...
export class GUIPick {
  ...
  private onTouchUp(e: PointerEvent3D) {
    ...
    if (collider && collider == this._lastDownTarget) {
      if (Time.time - this._lastDownTime <= this._clickTimeSpan) {
        this._calcDistanceVec2.set(e.mouseX, e.mouseY);
        if (
          this._calcDistanceVec2.distance(this._lastDownPosition) <=
          this._clickDistanceSpan
        ) {
  ...
  private _iteractive2PanelDict: Map<IUIInteractive, UIPanel> = new Map<
    IUIInteractive,
    UIPanel
  >();
  ...
  private collectEntities(): IUIInteractive[] {
    ...
    reversedCanvasList.forEach((canvas) => {
      if (canvas && canvas.transform && canvas.transform.parent) {
        let panels = canvas.object3D.getComponentsByProperty(
          "isUIPanel",
          true,
          true
        ) as UIPanel[];
  ...
  private pick(colliders: IUIInteractive[]): GUIHitInfo {
    this._ray = this._view.camera.screenPointToRay(
    ...
    for (const iterator of colliders) {
      if (
        iterator.interactive &&
        iterator.enable &&
        iterator.interactiveVisible
      ) {
