...
export class GUIGeometry extends GeometryBase {
  ...
  public create(): this {
    ...
    for (
      let i = 0, count = this.maxQuadCount * QuadStruct.vertexCount;
      i < count;
      i++
    ) {
      vIndexList[i] = i;
    ...
    this._attributeUV = new Float32Array(
      QuadStruct.vertexCount * quadNum * this._uvSize
    );
    this._attributeVIndex = new Float32Array(
      QuadStruct.vertexCount * quadNum * this._vIndexSize
    );
  ...
  private fillQuadPosition(quad: GUIQuad, transform: UITransform): void {
    SetBufferDataV4.setXYZW(
      this._posAttribute.array,
      quad.z,
      quad.left,
      quad.bottom,
      quad.right,
  ...
  private fillQuadSprite(quad: GUIQuad, transform: UITransform) {
    ...
    if (texture.isSliced && quad.imageType == ImageType.Sliced) {
      ...
      uvSliceWidth =
        (transform.width - (texture.offsetSize.z - texture.trimSize.x)) /
        uvSliceWidth;

      uvSliceHeight = texture.trimSize.y;
      uvSliceHeight =
        (transform.height - (texture.offsetSize.w - texture.trimSize.y)) /
        uvSliceHeight;
    }
