...
export class GUIQuad {
  ...
  public applyTransform(transform: UITransform): this {
    ...
    if (isSliced) {
      ...
      this._globalWidth =
        matrixScaleX *
        (transform.width - (item.offsetSize.z - item.trimSize.x));
      this._globalHeight =
        matrixScaleY *
        (transform.height - (item.offsetSize.w - item.trimSize.y));
      ...
      this._globalX =
        _worldMatrix.tx + item.offsetSize.x * transformScaleX * matrixScaleX;
      this._globalY =
        _worldMatrix.ty + item.offsetSize.y * transformScaleY * matrixScaleY;
    }
    return this;
  }
