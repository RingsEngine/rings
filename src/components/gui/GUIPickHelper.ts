...
export class GUIPickHelper {
  ...
  public static rayPick(
    ...
    if (space == GUISpace.World) {
      ...
      pickPoint = helpRay.intersectTriangle(
        helpRay.origin,
        helpRay.direction,
        this._triangle
      );
      ...
      let minX: number =
        Math.min(this._pt0.x, this._pt1.x, this._pt2.x, this._pt3.x) +
        screenSizeX * 0.5;
      let minY: number =
        Math.min(this._pt0.y, this._pt1.y, this._pt2.y, this._pt3.y) +
        screenSizeY * 0.5;
      let maxX: number =
        Math.max(this._pt0.x, this._pt1.x, this._pt2.x, this._pt3.x) +
        screenSizeX * 0.5;
      let maxY: number =
        Math.max(this._pt0.y, this._pt1.y, this._pt2.y, this._pt3.y) +
        screenSizeY * 0.5;
      let pick =
        screenPos.x <= maxX &&
        screenPos.x >= minX &&
  ...
  private static calculateHotArea_World(
    transform: UITransform,
    pt0: Vector3,
    pt1: Vector3,
    pt2: Vector3,
