import {
  IObject3DForPropertyAnim,
  Object3D,
} from "../../../core/entities/Object3D";
...
export class AnimationMonitor {
  ...
  public update(time: number, delta: number) {
    ...
    this.validProperty();
    if (
      this._currentClip.wrapMode != WrapMode.Loop &&
      this._currentClip.wrapMode != WrapMode.Default
    ) {
      let complete =
        this.speed > 0
          ? this._time >= this._currentClip.totalTime
          : this._time <= 0;
      if (complete) {
        this._isPlaying = false;
        this._animation["statusCall"](
          AnimationMonitor.Complete,
          lastTime,
          this._time
        );
      }
  ...
  private applyProperty(tag: PropertyAnimTag, obj3d: Object3D) {
    if (tag.quaternion) {
      Matrix4.getEuler(
        obj3d.transform.localRotation,
        obj3d.transform.localRotQuat,
        true,
        "ZYX"
      );
    }
    ...
    let animObj: IObject3DForPropertyAnim =
      obj3d as any as IObject3DForPropertyAnim;
