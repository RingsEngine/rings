...
export class MorphTarget_shader {
  public static getMorphTargetShaderBinding(
    group: number,
    beginBinding: number
  ): string {
    return `
            ...
            struct MorphTargetConfigData {
                morphBaseInfluence:f32, // 基础形变权重
                morphTargetCount:f32, // 变形目标数量
                totalVertexCount:f32, // 总顶点数
                computeWorkGroupXY:f32, // 计算工作组的XY维度
            };
            ...
            var<storage,read> morphTargetOpPosition: array<f32>;
