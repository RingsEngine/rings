...
export class MorphTargetBlender extends ComponentBase {
  ...
  public init(param?: any): void {
    let meshRenders: SkinnedMeshRenderer2[] = this.fetchMorphRenderers(
      this.object3D,
      SkinnedMeshRenderer2
    );
    let meshRenders2: MeshRenderer[] = this.fetchMorphRenderers(
      this.object3D,
      MeshRenderer
    );
  ...
  public applyBlendShape(
    frame: MorphTargetFrame,
    keyMapper: MorphTargetMapper,
    multiplier: number = 1
  ): void {
    if (!frame) {
  ...
  private applyMorphTargetInfluence(
    key: string,
    influence: number,
    rendererList: SkinnedMeshRenderer2[]
  ): void {
  ...
  private fetchMorphRenderers<T extends MeshRenderer>(
    obj: Object3D,
    c: Ctor<T>
  ): T[] {
