  ...
  public apply(vertexCount: number): void {
    if (this.source) {
      if (!this.input) {
        ...
        this.input = new StorageGPUBuffer(
          this.source.length,
          usage,
          this.source
        );
...
export class MorphTargetData {
  ...
  public applyRenderShader(renderShader: RenderShaderPass) {
    ...
    renderShader.setUniformBuffer(
      "morphTargetConfig",
      this._computeConfigBuffer
    );
    renderShader.setStorageBuffer(
      "morphTargetOpPositions",
      this._positionAttrDataGroup.output
    );

    if (this._collectMorphTargetData.mergedNormal) {
      renderShader.setStorageBuffer(
        "morphTargetOpNormals",
        this._normalAttrDataGroup.output
      );
    }
  }
  ...
  public computeMorphTarget(command: GPUCommandEncoder): void {
    ...
    this._computeShader.setStorageBuffer(
      ...
      this._morphInfluenceBuffer
    );
    this._computeShader.setStorageBuffer(
      "morphTargetPositions",
      this._positionAttrDataGroup.input
    );
    this._computeShader.setStorageBuffer(
      "morphTargetOpPositions",
      this._positionAttrDataGroup.output
    ...
    if (this._collectMorphTargetData.mergedNormal) {
      this._computeShader.setStorageBuffer(
        "morphTargetNormals",
        this._normalAttrDataGroup.input
      );
      this._computeShader.setStorageBuffer(
        "morphTargetOpNormals",
        this._normalAttrDataGroup.output
  ...
  private collectMorphTargetList(
    ...
    if (geometry.blendShapeData) {
      for (let i = 0; i < geometry.blendShapeData.shapeIndexs.length; i++) {
        ...
        this._blendTarget[shapeName] = (value) =>
          this.updateInfluence(index, value);
      }
    }

    let posArray: Float32Array = new Float32Array(
      vertexCount * morphTargetCount * 3
    );
    {
      let offset: number = 0;
    ...
    return {
      mtCount: morphTargetCount,
      vCount: vertexCount,
      mergedPos: posArray,
      mergedNormal: normalArray,
    };
  }
  ...
  private uploadConfigGBuffer(): void {
    if (this._isInfluenceDirty) {
      ...
      this._morphInfluenceBuffer.setFloat32Array(
        "data",
        this._morphInfluenceArray
      );
      this._morphInfluenceBuffer.apply();
      this._computeConfigArray[0] = this.morphTargetsRelative
        ? 1
        : 1 - sumInfluence;
      ...
      this._computeConfigBuffer.setFloat32Array(
        "data",
        this._computeConfigArray
      );
  ...
  protected generateGPUBuffer() {
    ...
    let usage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST;
    this._morphInfluenceBuffer = new StorageGPUBuffer(
      this.MaxMorphTargetCount,
      usage
    );
  }
}
