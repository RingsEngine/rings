...
export class OrbitController extends ComponentBase {
  ...
  public onUpdate() {
    ...
    this._cPosition.x =
      Math.abs(x) > 1e-10 ? this._cPosition.x + x : this._position.x;
    this._cPosition.y =
      Math.abs(y) > 1e-10 ? this._cPosition.y + y : this._position.y;
    this._cPosition.z =
      Math.abs(z) > 1e-10 ? this._cPosition.z + z : this._position.z;
    this._camera.lookAt(this._cPosition, this._cTarget, Vector3.UP);
  }
  /**
   * @internal
  ...
  private onPointerMove(e: PointerEvent3D) {
    ...
    if (e.mouseCode === 0 && this._lastMouseX > 0 && this._lastMouseY > 0) {
      ...
      Vector3Ex.mulScale(
        this.object3D.transform.up,
        e.movementY * this.panFactor * this._camera.aspect,
        Vector3.HELP_1
      );
      ...
      Vector3Ex.mulScale(
        this.object3D.transform.right,
        -e.movementX * this.panFactor,
        Vector3.HELP_1
      );
      this._target.x -= Vector3.HELP_1.x;
      this._target.z -= Vector3.HELP_1.z;
