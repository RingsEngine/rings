...
} from "../textures/AtmosphericScatteringSky";
...
class HistoryData {
  ...
  public isRotateChange(rx: number, ry: number): boolean {
    return (
      Math.abs(this.rotateX - rx) >= 0.001 ||
      Math.abs(this.rotateY - ry) >= 0.001
    );
  }
...
export class AtmosphericComponent extends SkyRenderer {
  ...
  public init(): void {
    ...
    this._atmosphericScatteringSky = new AtmosphericScatteringSky(
  ...
  public onUpdate(view?: any) {
    if (this._relatedTransform) {
      ...
      if (
        this._historyData.isRotateChange(
          this._relatedTransform.rotationX,
