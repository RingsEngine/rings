...
export class LightBase extends ComponentBase implements ILight {
  ...
  public init(): void {
    this.transform.object3D.bound = new BoundingBox(
      new Vector3(),
      new Vector3()
    );
  ...
  protected onChange() {
    ...
    this.transform.object3D.bound.setFromCenterAndSize(
    ...
    if (this.transform.view3D && Engine3D.renderJobs) {
      let renderer = Engine3D.renderJobs.get(
        this.transform.view3D
      ).reflectionRenderer;
      if (renderer)
        Engine3D.renderJobs
          .get(this.transform.view3D)
          .reflectionRenderer.forceUpdate();
    }
  }
  ...
  public destroy(force?: boolean): void {
    ...
    this.transform.eventDispatcher.removeEventListener(
      Transform.ROTATION_ONCHANGE,
      this.onRotChange,
      this
    );
    this.transform.eventDispatcher.removeEventListener(
      Transform.SCALE_ONCHANGE,
      this.onScaleChange,
      this
    );
    super.destroy(force);
