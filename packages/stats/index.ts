import { ComponentBase } from "@rings/core";
...
export class Stats extends ComponentBase {
  ...
  init() {
    ...
    container.setAttribute(
      "style",
      "display:flex;flex-direction:column;gap:1px;position:fixed;top:0;left:0;cursor:pointer;opacity:0.9;z-index:10000"
    );
...
class Panel {
  ...
  constructor(parent: HTMLElement, name: string, fg: string, bg: string) {
    ...
    this.worker.postMessage({ type: "init", offscreen, name, fg, bg }, [
      offscreen,
    ]);
  }
  ...
  onmessage = (e) => {
    if (e.data.type == "update") {
      ...
      context.fillRect(
        ...
        GRAPH_Y,
        PR,
        Math.round((1 - e.data.value / e.data.maxValue) * GRAPH_HEIGHT)
      );
