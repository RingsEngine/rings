...
export class RopeSoftbody extends SoftbodyBase {
  ...
  async start(): Promise<void> {
    if (this.anchorRigidbodyHead) {
      ...
      this._positionHead = TempPhyMath.fromBtVec(
        bodyA.getWorldTransform().getOrigin()
      );
    ...
    if (this.anchorRigidbodyTail) {
      ...
      this._positionTail = TempPhyMath.fromBtVec(
  ...
  protected initSoftBody(): Ammo.btSoftBody {
    const vertexArray = this._geometry.getAttribute(
      VertexAttributeName.position
    ).data;

    this._positionHead ||= new Vector3(
      vertexArray[0],
      vertexArray[1],
      vertexArray[2]
    );
    this._positionTail ||= new Vector3(
      vertexArray.at(-3),
      vertexArray.at(-2),
      vertexArray.at(-1)
    );

    const ropeStart = TempPhyMath.toBtVec(
      this._positionHead,
      TempPhyMath.tmpVecA
    );
    const ropeEnd = TempPhyMath.toBtVec(
      this._positionTail,
      TempPhyMath.tmpVecB
    );
   ...
   */
  public static buildRopeGeometry(
    segmentCount: number,
    startPos: Vector3,
    endPos: Vector3
  ): GeometryBase {
