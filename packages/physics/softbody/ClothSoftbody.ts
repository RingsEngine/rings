...
export class ClothSoftbody extends SoftbodyBase {
  ...
  protected initSoftBody(): Ammo.btSoftBody {
    ...
    if (!this.clothCorners) {
      ...
      clothCorner00 = TempPhyMath.toBtVec(
        this.clothCorners[0],
        TempPhyMath.tmpVecA
      );
      clothCorner01 = TempPhyMath.toBtVec(
        this.clothCorners[1],
        TempPhyMath.tmpVecB
      );
      clothCorner10 = TempPhyMath.toBtVec(
        this.clothCorners[2],
        TempPhyMath.tmpVecC
      );
      clothCorner11 = TempPhyMath.toBtVec(
        this.clothCorners[3],
        TempPhyMath.tmpVecD
      );
    }
  ...
  protected configureSoftBody(clothSoftbody: Ammo.btSoftBody): void {
    ...
    clothSoftbody.generateBendingConstraints(
      2,
      clothSoftbody.get_m_materials().at(0)
    );
    ...
    if (this.anchorIndices.length > 0) {
      ...
      clothSoftbody.rotate(
        TempPhyMath.eulerToBtQua(this.transform.localRotation)
      );
      clothSoftbody.translate(
        TempPhyMath.toBtVec(this.transform.localPosition)
      );
    }
  }
  ...
  private applyAnchor(clothSoftbody: Ammo.btSoftBody): void {
    ...
    Quaternion.HELP_1.fromEulerAngles(
