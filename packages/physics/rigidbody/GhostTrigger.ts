import { ComponentBase, Vector3 } from "@rings/core";
...
export class GhostTrigger extends ComponentBase {
  ...
  async start() {
    ...
    this._ghostObject = GhostTrigger.createAndAddGhostObject(
      this._shape,
      position,
      rotation,
    ...
    this.transform.onPositionChange = (
      oldValue: Vector3,
      newValue: Vector3
    ) => {
      ...
      this._ghostObject
        .getWorldTransform()
        .setOrigin(TempPhyMath.toBtVec(newValue));
    };
    this.transform.onRotationChange = (
      oldValue: Vector3,
      newValue: Vector3
    ) => {
      ...
      this._ghostObject
        .getWorldTransform()
        .setRotation(TempPhyMath.eulerToBtQua(newValue));
    };
   ...
   */
  public static createAndAddGhostObject(
    ...
    ghostObject.setCollisionFlags(
      ghostObject.getCollisionFlags() | collisionFlags
    );
