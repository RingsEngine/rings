...
export class Rigidbody extends ComponentBase {
  ...
  private _initializationPromise: Promise<void> = new Promise<void>(
    (r) => (this._initResolve = r)
  );
  ...
  private initRigidbody(): void {
    ...
    this._btRigidbody = RigidBodyUtil.createRigidBody(
      this.object3D,
      this._shape,
      this._mass,
      position
    ...
    if (this._contactProcessingThreshold != null) {
      this._btRigidbody.setContactProcessingThreshold(
  ...
  public onUpdate(): void {
    ...
    if (this._btRigidbody?.isActive()) {
      ...
      this._btRigidbody
        .getMotionState()
        .getWorldTransform(Physics.TEMP_TRANSFORM);
      ...
      this.transform.localPosition = Vector3.HELP_0.set(
        pos.x(),
        pos.y(),
        pos.z()
      );
      this.transform.localRotQuat = Quaternion.HELP_0.set(
        qua.x(),
        qua.y(),
        qua.z(),
        qua.w()
    ...
    if (this._btRigidbody) {
      ...
      if (value instanceof Ammo.btHeightfieldTerrainShape) {
        this._btRigidbody
          .getWorldTransform()
          .getOrigin()
          .setY((value as any).averageHeight + this.object3D.y);
      }
    }
  }
  ...
  public set isKinematic(value: boolean) {
    ...
    if (value) {
      // pause onUpdate
      this.enable = false;
      this._btRigidbody.setActivationState(
      ...
      const state =
        ...
        (this._btRigidbody.isStaticObject()
  ...
  public get isDisableDebugVisible(): boolean {
    return (
      (this.collisionFlags & CollisionFlags.DISABLE_VISUALIZE_OBJECT) !== 0
    );
  }
  /**
  ...
  public set velocity(value: Vector3) {
    ...
    this.wait().then((rb) =>
      rb.applyForce(
        TempPhyMath.toBtVec(this._velocity),
        TempPhyMath.zeroBtVec(TempPhyMath.tmpVecB)
      )
    );
  }
  ...
  public set angularVelocity(value: Vector3) {
    ...
    this.wait().then((rb) =>
      rb.setAngularVelocity(TempPhyMath.toBtVec(this._angularVelocity))
    );
  }
  /**
  ...
  public set linearVelocity(value: Vector3) {
    ...
    this.wait().then((rb) =>
      rb.setLinearVelocity(TempPhyMath.toBtVec(this._linearVelocity))
    );
  }
  /**
   * Get mass value
