...
export class HingeConstraint extends ConstraintBase<Ammo.btHingeConstraint> {
  ...
  protected createConstraint(
    ...
    const constraintType = !targetBody
      ? "SINGLE_BODY_TRANSFORM"
      : this.useTwoBodiesTransformOverload
      ? "TWO_BODIES_TRANSFORM"
      : "TWO_BODIES_PIVOT";
    ...
    switch (constraintType) {
      case "SINGLE_BODY_TRANSFORM":
        ...
        this._constraint = new Ammo.btHingeConstraint(
          selfBody,
          frameA_single,
      ...
      case "TWO_BODIES_TRANSFORM":
        ...
        frameB.setRotation(
          TempPhyMath.toBtQua(this.rotationTarget, TempPhyMath.tmpQuaB)
        );
      ...
      case "TWO_BODIES_PIVOT":
        const axisSelf = TempPhyMath.toBtVec(
          this.axisSelf,
          TempPhyMath.tmpVecC
        );
        const axisTarget = TempPhyMath.toBtVec(
          this.axisTarget,
          TempPhyMath.tmpVecD
        );
    ...
    this._pendingMotorConfig &&
      this.enableAngularMotor(...this._pendingMotorConfig);
  }
}
