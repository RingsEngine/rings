...
export class Generic6DofSpringConstraint extends ConstraintBase<Ammo.btGeneric6DofSpringConstraint> {
  ...
  private _angularLowerLimit: Vector3 = new Vector3(
    -Math.PI,
    -Math.PI,
    -Math.PI
  );
  ...
  protected createConstraint(
    ...
    targetBody: Ammo.btRigidBody | null
  ): void {
    ...
    if (targetBody) {
      const pivotInB = TempPhyMath.toBtVec(
        this.pivotTarget,
        TempPhyMath.tmpVecB
      );
      const rotInB = TempPhyMath.toBtQua(
        this.rotationTarget,
        TempPhyMath.tmpQuaB
      );
  ...
  private setConstraint() {
    ...
    this._constraint.setLinearLowerLimit(
      TempPhyMath.toBtVec(this._linearLowerLimit)
    );
    this._constraint.setLinearUpperLimit(
      TempPhyMath.toBtVec(this._linearUpperLimit)
    );
    this._constraint.setAngularLowerLimit(
      TempPhyMath.toBtVec(this._angularLowerLimit)
    );
    this._constraint.setAngularUpperLimit(
      TempPhyMath.toBtVec(this._angularUpperLimit)
    );

    // 应用缓存的弹簧参数
    this._springParams.forEach((param) =>
      this._constraint.enableSpring(param.index, param.onOff)
    );
    this._stiffnessParams.forEach((param) =>
      this._constraint.setStiffness(param.index, param.stiffness)
    );
    this._dampingParams.forEach((param) =>
      this._constraint.setDamping(param.index, param.damping)
    );
    this._equilibriumPointParams.forEach((param) =>
      this.setEquilibriumPoint(param.index, param.val)
    );
