...
export class ContactProcessedUtil {
   ...
   * @param callback 事件回调
   */
  public static registerCollisionCallback(
    pointer: number,
    callback: Callback
  ): void {
  ...
  private static registerContactProcessedCallback(): void {
    if (ContactProcessedUtil.contactProcessedCallbackPointer === null) {
      ContactProcessedUtil.contactProcessedCallbackPointer = Ammo.addFunction(
        ContactProcessedUtil.contactProcessedCallback
      );
      Physics.world.setContactProcessedCallback(
        ContactProcessedUtil.contactProcessedCallbackPointer
      );
    }
  }
  ...
  private static contactProcessedCallback(
    ...
    // 检查是否需要忽略
    if (
      ContactProcessedUtil.ignoredPointers.has(colObj0WrapPtr) ||
      ContactProcessedUtil.ignoredPointers.has(colObj1WrapPtr)
    ) {
      return 0;
    }
   ...
   */
  public static performCollisionTest(
    bodyA: Ammo.btRigidBody,
    bodyB?: Ammo.btRigidBody
  ) {
    ...
    callback.addSingleResult = (
      ...
      let collisionObjectWrapperA = Ammo.wrapPointer(
        colObj0Wrap as unknown as number,
        Ammo.btCollisionObjectWrapper
      );
      let collisionObjectWrapperB = Ammo.wrapPointer(
        colObj1Wrap as unknown as number,
        Ammo.btCollisionObjectWrapper
      );
      collisionDetected = {
