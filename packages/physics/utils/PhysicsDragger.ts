...
export class PhysicsDragger {
  ...
  private initRaycast() {
    ...
    this._raycastResult = new Ammo.ClosestRayResultCallback(
      this._rayStart,
      this._rayEnd
    );
  }
  ...
  private onMouseDown(e: PointerEvent3D) {
    ...
    if (e.mouseCode === 0) {
      ...
      let endPos = ray.origin.add(
        adjustedDirection.multiplyScalar(1000),
        ray.origin
      );
      ...
      if (this._isDragging) {
        ...
        const worldCoordinates = camera.worldToScreenPoint(
          this._hitPoint,
          Vector3.HELP_1
        );
  ...
  private castRay(cameraPos: Vector3, targetPos: Vector3) {
    ...
    if (this._raycastResult.hasHit()) {
      ...
      // 交点
      TempPhyMath.fromBtVec(
        this._raycastResult.get_m_hitPointWorld(),
        this._hitPoint
      );

      this._rigidBody.setCollisionFlags(
        this._rigidBody.getCollisionFlags() | CollisionFlags.KINEMATIC_OBJECT
      );
      ...
      let originPos = TempPhyMath.fromBtVec(
        Physics.TEMP_TRANSFORM.getOrigin(),
        Vector3.HELP_0
      );
  ...
  private updateRigidBody() {
    let pos = this._view.camera.screenPointToWorld(
    ...
    this._rigidBody
      .getWorldTransform()
      .setOrigin(Physics.TEMP_TRANSFORM.getOrigin()); // 确保静态刚体的位置信息是同步的
