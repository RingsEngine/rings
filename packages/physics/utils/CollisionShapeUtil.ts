...
export class CollisionShapeUtil {
   ...
   */
  public static createStaticPlaneShape(
    planeNormal: Vector3 = Vector3.UP,
  ...
  public static createBoxShape(object3D: Object3D, size?: Vector3) {
    ...
    const halfExtents = TempPhyMath.setBtVec(
      size.x / 2,
      size.y / 2,
      size.z / 2
    );
   ...
   */
  public static createCapsuleShape(
    object3D: Object3D,
    radius?: number,
    height?: number
  ) {
    ...
    if (!radius || !height)
      boundSize = this.calculateLocalBoundingBox(object3D).size;
   ...
   */
  public static createCylinderShape(
    object3D: Object3D,
    radius?: number,
    height?: number
  ) {
   ...
   * @returns 复合碰撞形状。
   */
  public static createCompoundShapeFromObject(
    object3D: Object3D,
    ...
    object3D.forChild((child: Object3D) => {
      ...
      if (shape) {
        ...
        const position = new Vector3();
        const rotation = new Quaternion();
        localMatrix.decompose("quaternion", [
          position,
          rotation as any,
          Vector3.HELP_0,
        ]);
   ...
   */
  public static createShapeFromObject(
    object3D: Object3D
  ): Ammo.btCollisionShape | null {
  ...
  public static createHeightfieldTerrainShape(
    ...
    let ammoHeightDataF32 = new Float32Array(
      Ammo.HEAPF32.buffer,
      ammoHeightData,
      heightData.length
    );
    ...
    let localScaling = TempPhyMath.setBtVec(
      width / segmentW,
      1,
      height / segmentH
    );
   ...
   */
  public static createConvexHullShape(
    object3D: Object3D,
    modelVertices?: Float32Array
  ) {
    let vertices =
      modelVertices || this.getAllMeshVerticesAndIndices(object3D).vertices;

    let shape = new Ammo.btConvexHullShape();
    for (let i = 0, count = vertices.length / 3; i < count; i++) {
      let point = TempPhyMath.setBtVec(
        vertices[3 * i],
        vertices[3 * i + 1],
        vertices[3 * i + 2]
      );
   ...
   */
  public static createConvexTriangleMeshShape(
    object3D: Object3D,
    modelVertices?: Float32Array,
   ...
   */
  public static createBvhTriangleMeshShape(
    object3D: Object3D,
    modelVertices?: Float32Array,
    modelIndices?: Uint16Array
  ): Ammo.btBvhTriangleMeshShape {
   ...
   */
  public static createGImpactMeshShape(
    object3D: Object3D,
    modelVertices?: Float32Array,
    modelIndices?: Uint16Array
   ...
   */
  public static buildTriangleMesh(
    ...
    for (let i = 0; i < indices.length; i += 3) {
      ...
      const v0 = TempPhyMath.setBtVec(
        vertices[index0],
        vertices[index0 + 1],
        vertices[index0 + 2],
        TempPhyMath.tmpVecA
      );
      const v1 = TempPhyMath.setBtVec(
        vertices[index1],
        vertices[index1 + 1],
        vertices[index1 + 2],
        TempPhyMath.tmpVecB
      );
      const v2 = TempPhyMath.setBtVec(
        vertices[index2],
        vertices[index2 + 1],
        vertices[index2 + 2],
        TempPhyMath.tmpVecC
   ...
   * @returns 顶点数据和索引数据。
   */
  public static getAllMeshVerticesAndIndices(
    object3D: Object3D,
    isTransformChildren: boolean = true
  ) {
    let meshRenderers = object3D.getComponents(MeshRenderer);

    if (meshRenderers.length === 1 && !isTransformChildren) {
      return {
        vertices: meshRenderers[0].geometry.getAttribute(
          VertexAttributeName.position
        ).data as Float32Array,
        indices: meshRenderers[0].geometry.getAttribute(
          VertexAttributeName.indices
        ).data as Uint16Array,
      };
    }
    ...
    meshRenderers.forEach((renderer) => {
      totalVertexLength += renderer.geometry.getAttribute(
        VertexAttributeName.position
      ).data.length;
      totalIndexLength += renderer.geometry.getAttribute(
        VertexAttributeName.indices
      ).data.length;
    });
    ...
    meshRenderers.forEach((renderer) => {
      let vertexArray = renderer.geometry.getAttribute(
        VertexAttributeName.position
      ).data;
      ...
      let indexArray = renderer.geometry.getAttribute(
        VertexAttributeName.indices
      ).data;
