...
export class PhysicsDebugDrawer {
  ...
  private drawLine(
    ...
    const fromVector = Ammo.wrapPointer(
      from as unknown as number,
      Ammo.btVector3
    );
    const toVector = Ammo.wrapPointer(to as unknown as number, Ammo.btVector3);
    const colorVector = Ammo.wrapPointer(
      color as unknown as number,
      Ammo.btVector3
    );

    const lineColor = this._tmpCor.copyFromVector(
      TempPhyMath.fromBtVec(colorVector, this._tmpVecA)
    );
  ...
  private drawContactPoint(
    ...
    const pointOnBVector = Ammo.wrapPointer(
      pointOnB as unknown as number,
      Ammo.btVector3
    );
    const normalOnBVector = Ammo.wrapPointer(
      normalOnB as unknown as number,
      Ammo.btVector3
    );
  ...
  private draw3dText(location: Ammo.btVector3, textString: string): void {
    const _location = Ammo.wrapPointer(
      location as unknown as number,
      Ammo.btVector3
    );
