...
class _Physics {
  ...
  public async init(
    ...
    this._physicBound = new BoundingBox(
      new Vector3(),
      options.physicBound || new Vector3(2000, 2000, 2000)
    );
  }
  ...
  private initWorld(useSoftBody: boolean) {
    ...
    if (useSoftBody) {
      ...
      this._world = new Ammo.btSoftRigidDynamicsWorld(
        dispatcher,
        broadphase,
        solver,
      ...
      this._worldInfo = (
        this.world as Ammo.btSoftRigidDynamicsWorld
      ).getWorldInfo();
  ...
  public syncGraphic(object3D: Object3D, tm: Ammo.btTransform): void {
    object3D.localPosition = TempPhyMath.fromBtVec(
      tm.getOrigin(),
      Vector3.HELP_0
    );
    object3D.localQuaternion = TempPhyMath.fromBtQua(
      tm.getRotation(),
      Quaternion.HELP_0
    );
  }
