...
export function CreateFloatArray(
...
export class WasmMatrix {
  ...
  public static allocMatrix(count: number) {
    if (count > Matrix4.maxCount) {
      console.error(
        `The maximum allocation size is exceeded! current:${count}, limit:${Matrix4.maxCount}`
      );
    }
    ...
    if (this.useDoublePrecision) {
      this.matrixBuffer = CreateFloatArray(
        this.wasm.HEAPF64.buffer,
        this.matrixBufferPtr,
        16 * count
      );
      this.matrixSRTBuffer = CreateFloatArray(
        this.wasm.HEAPF64.buffer,
        this.matrixSRTBufferPtr,
        3 * 3 * count
      );
      this.matrixContinuedSRTBuffer = CreateFloatArray(
        this.wasm.HEAPF64.buffer,
        this.matrixContinuedSRTBufferPtr,
        3 * 3 * count
      );
    ...
    this.matrixStateBuffer = new Int32Array(
      this.wasm.HEAP32.buffer,
      this.matrixStateBufferPtr,
      WasmMatrix.stateStruct * count
    );

    Matrix4.allocMatrix(count);
  }

  public static updateAllContinueTransform(
    start: number,
    end: number,
    dt: number
  ) {
  ...
  public static setParent(matIndex: number, x: number, depthOrder: number) {
    this.matrixStateBuffer[matIndex * WasmMatrix.stateStruct + 2] =
      x >= 0 ? x : -1;
    this.matrixStateBuffer[matIndex * WasmMatrix.stateStruct + 3] = depthOrder;
  }

  public static setTranslate(
    matIndex: number,
    x: number,
    y: number,
    z: number
  ...
  public static setContinueTranslate(
    matIndex: number,
    x: number,
    y: number,
    z: number
  ...
  public static setContinueRotation(
    matIndex: number,
    x: number,
    y: number,
    z: number
  ...
  public static setContinueScale(
    matIndex: number,
    x: number,
    y: number,
    z: number
